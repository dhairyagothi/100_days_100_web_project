...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  updateNavbar();
  initScrollBtn();
  // Load cached repository stats first (generated JSON), then fall back to live GitHub API
  await loadLocalRepoStats().catch(() => {});
  fetchRepoStats();
  ...
  try {
    ...
    syncProjectCounts();
    // try to load cached stats again if needed
    await loadLocalRepoStats().catch(() => {});
    fetchRepoStats();
    initScrollBtn();
...
/**
 * Load repository-stats.json generated by the repo-stats script and render insights.
 * Returns a promise that resolves when data is processed or rejects on fetch error.
 */
async function loadLocalRepoStats() {
  try {
    const res = await fetch(new URL('./repository-stats.json', window.location.href));
    if (!res.ok) throw new Error('No local stats');
    const stats = await res.json();
    renderRepoInsights(stats);
  } catch (e) {
    // graceful failure — consumer will fall back to live GitHub API
    throw e;
  }
}

function renderRepoInsights(stats) {
  try {
    ...
    setText('riTotalProjects', stats.total_projects?.toLocaleString?.() ?? stats.total_projects ?? '—');

    const categoryCount = stats.by_type ? Object.keys(stats.by_type).length : '—';
    setText('riCategoryCount', categoryCount);

    const topTech = Array.isArray(stats.top_tech_tags) && stats.top_tech_tags.length > 0
      ? stats.top_tech_tags[0].tag
      : (Object.keys(stats.tech_tag_counts || {})[0] || '—');
    setText('riTopTech', topTech);

    setText('riExternalDemos', stats.external_demos ?? '—');

    // Category distribution: render small list with counts
    const catList = document.getElementById('riCategoryList');
    if (catList && stats.by_type) {
      catList.innerHTML = '';
      const entries = Object.entries(stats.by_type).sort((a,b)=>b[1]-a[1]);
      entries.forEach(([cat, count]) => {
        const item = document.createElement('div');
        item.className = 'category-item';
        item.innerHTML = `<span class="cat-name">${capitalize(cat)}</span><span class="cat-count">${count}</span>`;
        catList.appendChild(item);
      });
    }
  } catch (e) {
    console.error('Failed to render repo insights', e);
  }
}
