...
function buildProjectCardHTML({
  ...
  const tagsHTML = tagsArray
    .map((t) => {
      const techLogo = TECH_LOGOS[t.toLowerCase()] || "";
      return `<span class="tag">${techLogo}${escapeHTML(t)}</span>`;
    })
    .join("");
...
function updateTechFilterDisplay() {
  ...
  techStackFilters.forEach((tech) => {
    ...
    const techLogo = TECH_LOGOS[tech.toLowerCase()];
    if (techLogo) {
      const logoWrap = document.createElement("span");
      logoWrap.className = "tech-logo-wrap";
      logoWrap.innerHTML = techLogo;
      span.appendChild(logoWrap);
    }
