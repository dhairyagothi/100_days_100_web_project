...
const { logEvent } = require("./securityMonitor");
...
module.exports = async (req, res) => {
  ...
  if (!model || !Array.isArray(contents) || contents.length === 0) {
    logEvent(clientIp, "INVALID_REQUEST");
  ...
  if (!ALLOWED_MODELS.has(model)) {
    ...
    return res.status(400).json({
      error: `Model '${model}' is not permitted. Allowed models: ${[
        ...ALLOWED_MODELS,
      ].join(", ")}`,
    });
  }

  const MAX_CONTENTS_BYTES = 32 * 1024;
  ...
  try {
    ...
    logEvent(clientIp, "SERVER_ERROR");
