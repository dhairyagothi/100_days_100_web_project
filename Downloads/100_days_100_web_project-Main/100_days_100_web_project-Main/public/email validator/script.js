...
function validateFormat(email) {
  ...
  if (local.length === 0) {
    ...
    if (local.startsWith('.')) errors.push('Local part (before @) cannot start with a dot (.)');
    if (local.endsWith('.')) errors.push('Local part (before @) cannot end with a dot (.)');
    if (/\.\./.test(local)) errors.push('Consecutive dots (..) are not allowed before the @');
    if (/\s/.test(local)) errors.push('Spaces are not allowed in the email address');
  ...
  if (domain.length === 0) {
    ...
    if (/\.\./.test(domain)) errors.push('Consecutive dots (..) are not allowed in the domain');
