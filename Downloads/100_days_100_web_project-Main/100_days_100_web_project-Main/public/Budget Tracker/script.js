...
function updateInsights() {
  ...
  if (maxAmount > 0) {
    const formattedCat =
      maxCat === 'other' ? 'Others' : maxCat.charAt(0).toUpperCase() + maxCat.slice(1);
  ...
  if (maxAmount > 0) {
    if (maxCat === 'food') {
      suggestion =
        'You spent more on food this month compared to other categories. Consider cooking at home to save money.';
    } else if (maxCat === 'travel') {
      suggestion =
        'You spent more on travel this month compared to other categories. Consider using public transport or carpooling.';
    } else if (maxCat === 'shopping') {
      suggestion =
        'You spent more on shopping this month compared to other categories. Try waiting 48 hours before buying non-essential items.';
