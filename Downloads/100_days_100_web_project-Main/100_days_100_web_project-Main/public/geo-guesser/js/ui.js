...
const UI = {
  ...
  showInlineResult(data) {
    ...
    if (resultInfo) {
      ...
      document.getElementById('inlineResultTitle').textContent = Utils.getResultTitle(
        data.distance
      );
  ...
  showFinalScreen(data) {
    ...
    document.getElementById('performanceMessage').textContent = Utils.getPerformanceMessage(
      data.finalScore,
      maxPossible
    );
  ...
  displayHints(location, count) {
    ...
    img.onerror = function () {
      ...
      const placeholder =
        this.parentElement.querySelector('.image-placeholder') || document.createElement('div');
