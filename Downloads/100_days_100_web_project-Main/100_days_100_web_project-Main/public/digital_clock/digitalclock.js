...
function renderCountryOptions(countries) {
  ...
  countries.forEach((country) => {
    ...
    optionRow.style.cssText =
      'display:flex; align-items:center; gap:12px; padding:10px; cursor:pointer; border-bottom:1px solid rgba(255,255,255,0.05); color:#fff;';
...
if (worldSearchInput) {
  worldSearchInput.addEventListener('input', (e) => {
    ...
    const filtered = countriesDatabase.filter((c) => c.name.common.toLowerCase().includes(term));
...
function renderHistoryLogs() {
  ...
  container.innerHTML = historyLogs.map((log) => `<div>${log.text}</div>`).join('');
}
