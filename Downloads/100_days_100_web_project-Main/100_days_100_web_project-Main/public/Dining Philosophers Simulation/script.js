...
var forks = [true, true, true, true, true]; // true = fork is free on tablevar simStarted    = false;var deadlockShown = false;
...
function checkDeadlockCondition() {
  ...
  for (var i = 0; i < 5; i++) {
    if (philStates[i] == 'Hungry') hungryCount++;
    if (forks[i] == true) forksAvailable++;
  }

  if (hungryCount == 5 && forksAvailable == 0 && deadlockShown == false) {
    deadlockShown = true;
    document.getElementById('alert').style.display = 'block';
    addLog('>>> DEADLOCK! All philosophers waiting, all forks held.');
  }
}
