...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const stored = getStoredEntries();
  if (stored.length === 0) {
    document.getElementById("gate-sub").textContent =
      "Set a master password to create your vault.";
    document.getElementById("gate-hint").textContent =
      "This password encrypts all your data. Don't forget it.";
  } else {
    document.getElementById("gate-sub").textContent =
      "Enter your master password to unlock the vault.";
    document.getElementById("gate-hint").textContent =
      `${stored.length} encrypted entr${stored.length === 1 ? "y" : "ies"} stored.`;
  }
});
...
function updateStrengthUI(level) {
  ...
  for (let i = 0; i < 5; i++) {
    ...
    seg.style.background =
      i <= level ? STRENGTH_COLORS[level] : "var(--border2)";
  }
