  ...
  function synthesizeVideoFrame() {
    ...
    for (let y = 0; y < height; y++) {
      for (let x = 0; x < width; x++) {
        ...
        const waveG =
          Math.cos(y * 0.02 * frequency + waveOffset) * (colorScale * 0.7);
        const waveB =
          Math.sin((x + y) * 0.01 * frequency + waveOffset) * 128 + 128;
    ...
    if (!isScrubbing) {
      animationId = requestAnimationFrame(synthesizeVideoFrame);
    } else {
      animationId = null;
    }
  }
