...
function getHoveredConstellation(mouseX, mouseY) {
    for (const constellation of constellationStars) {
        for (const point of constellation.points) {
            ...
            const distance = Math.hypot(mouseX - screenPoint.x, mouseY - screenPoint.y);
...
function drawConstellation(constellation, isActive = false) {
    ...
    ctx.strokeStyle = isActive ? constellation.color : 'rgba(148, 163, 184, 0.32)';
...
function handleWheel(event) {
    ...
    camera.x = worldBeforeZoom.x - (event.clientX - canvas.width / 2) / camera.scale;
    camera.y = worldBeforeZoom.y - (event.clientY - canvas.height / 2) / camera.scale;
}
...
function animate() {
    ...
    stars.forEach(star => {
        ...
        ctx.beginPath();
        ctx.arc(screenStar.x, screenStar.y, star.radius * camera.scale, 0, Math.PI * 2);
        ctx.fillStyle = `rgba(255, 255, 255, ${dynamicAlpha})`;
        ctx.fill();
    });

    constellationStars.forEach((constellation) => {
        const isActive = hoveredConstellation && hoveredConstellation.id === constellation.id;
