...
// ==========================================================================
// DYNAMIC TYPOGRAPHY ELEMENT CREATION (PR #1365 Fix)
// ==========================================================================
const changeSizeBtn = document.getElementById("changeSizeBtn");
const changeColorBtn = document.getElementById("changeColorBtn");
const saveNoteBtn = document.getElementById("saveNoteBtn");

// Create size dropdown dynamically
...
// Show/hide size dropdown near the button
if (changeSizeBtn) {
...
// Handle layout drop-down item clicks
...
// ==========================================================================
// TYPOGRAPHY / COLOR SELECTIONS (PR #1365 Content Area Restrictions)
// ==========================================================================
function applyTextColor(c) {
  ...
  if (selection.rangeCount > 0 && !selection.isCollapsed) {
    ...
    if (!noteContentEl.contains(range.commonAncestorContainer)) {
...
function applyFontSize(s) {
  ...
  if (selection.rangeCount > 0 && !selection.isCollapsed) {
    ...
    // Fallback inline wrapper styling for custom px heights
    const span = document.createElement("span");
    span.style.fontSize = s + 'px';
    range.surroundContents(span);
  } else {
    noteContentEl.style.fontSize = s + 'px';
  }
  triggerAutosave();
}
...
// Global shortcut mapping setup
document.addEventListener('keydown', (e) => {
  if ((e.ctrlKey || e.metaKey) && e.shiftKey && e.key === 'H') {
    e.preventDefault();
    if (activeId) removeHighlight();
  }
  if ((e.ctrlKey || e.metaKey) && e.shiftKey && e.key === 'C') {
    e.preventDefault();
    if (activeId) clearAllHighlights();
  }
});
