...
function renderNotesList(filter = "") {
  ...
  if (folderFilter)
    filtered = filtered.filter((n) => n.folderId === folderFilter);
  // Tag search: if query starts with #
  if (q.startsWith("#")) {
    const tag = q.slice(1).toLowerCase();
    filtered = filtered.filter(
      (n) => n.tags && n.tags.some((t) => t.toLowerCase().includes(tag)),
    );
  }
...
function restoreVersion(index) {
  ...
  openModal(
    "Restore Version",
    "This will overwrite your current note content. Continue?",
    () => {
...
function openAnalytics() {
  ...
  const now = new Date();
  const last7 = notes.filter(
    (n) => now - new Date(n.created) < 7 * 86400000,
  ).length;
  const last30 = notes.filter(
    (n) => now - new Date(n.created) < 30 * 86400000,
  ).length;
