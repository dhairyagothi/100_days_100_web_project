  ...
  cars.forEach((car) => {
    ...
    if (!car.passed) {
      ...
      if (clearRight || clearLeft) {
        car.passed = true;
        // UPDATED: Cars Passed metric counter tracking now boosts up by 5 per car
        carsPassed += 5;
        // UPDATED: Score value now boosts up by 25 points per car
        score += 25 + level * 2;
        updateHUD();
      }
    }
  ...
  for (let i = 0; i < cars.length; i++) {
    ...
    for (let j = i + 1; j < cars.length; j++) {
      ...
      if (
        ...
        collisions++;
        // UPDATED: Reduced penalty rate deduction on crash to just -5
...
function drawCar(car) {
  const { x, y, w, h, color, goRight, state, collided } = car;
  ctx.save();
  if (!goRight) {
