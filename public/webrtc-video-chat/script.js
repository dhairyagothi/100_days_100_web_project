document.addEventListener('DOMContentLoaded', () => {
  ...
  peer.on('call', (call) => {
    ...
    // Answer using the currently active stream
  ...
  screenShareBtn.addEventListener('click', async () => {
    if (!screenStream) {
      try {
        ...
        // Keep the active outgoing stream in sync for future calls
        if (!currentCall) {
  ...
  function updateNetworkBadge(data) {
    const [icon, text] = getQuality(data.rtt, data.packetLoss);
    ...
    document.getElementById('rtt').textContent = `${data.rtt.toFixed(0)} ms`;
