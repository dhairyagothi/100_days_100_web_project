...
document.querySelectorAll('.sound-btn').forEach((btn) => {
  btn.addEventListener('click', () => {
    ...
    if (activeSound === key) {
      ...
      sounds[key]
        .play()
        .catch((err) => console.warn('Audio play failed — check audio source URL:', err));
      activeSound = key;
...
document.addEventListener(
  ...
  (e) => {
    ...
    const x = Math.max(
      0,
      Math.min(window.innerWidth - sticky.offsetWidth, touch.clientX - dragOffX)
    );
    const y = Math.max(
      0,
      Math.min(window.innerHeight - sticky.offsetHeight, touch.clientY - dragOffY)
    );
    sticky.style.left = x + 'px';
    sticky.style.top = y + 'px';
    sticky.style.right = 'auto';
