...
const MODES = {
  focus: {
    ...
    sub: "stay locked in",
    ringColor: "#c4a4e8",
    glowColor: "rgba(180,140,230,0.3)",
  },
  short: {
    duration: 5 * 60,
    label: "Short Break",
    sub: "breathe & stretch",
    ringColor: "#7dd4c0",
    glowColor: "rgba(125,212,192,0.3)",
  },
  long: {
    duration: 15 * 60,
    label: "Long Break",
    sub: "you earned this",
    ringColor: "#f0c070",
    glowColor: "rgba(240,192,112,0.3)",
  },
};
  ...
  btn.addEventListener("click", () => {
    ...
    if (activeSound === key) {
      ...
      sounds[key]
        .play()
        .catch((err) =>
          console.warn("Audio play failed — check audio source URL:", err),
        );
      activeSound = key;
...
const PLAYLISTS = {
  ...
  classical:
    "https://open.spotify.com/embed/playlist/37i9dQZF1DWWEJlAGA9gs0?utm_source=generator&theme=0",
};
...
document.addEventListener("mousemove", (e) => {
  ...
  const x = Math.max(
    0,
    Math.min(window.innerWidth - sticky.offsetWidth, e.clientX - dragOffX),
  );
  const y = Math.max(
    0,
    Math.min(window.innerHeight - sticky.offsetHeight, e.clientY - dragOffY),
  );
