...
function generateBombs(safeCoord) {
  bomblist = [];
  ...
  while (bomblist.length < 4) {
    ...
    if (bombCoord !== safeCoord && !bomblist.includes(bombCoord)) {
...
function getNeighbours(coord) {
  ...
  let validCols = [colIndex - 1, colIndex + 1, colIndex].filter(
    (c) => c >= 0 && c < 5
  );
  let validRows = [rowIndex - 1, rowIndex + 1, rowIndex].filter(
    (r) => r >= 1 && r <= 5
  );
