...
function loadQuestion() {
  ...
  shuffled.forEach((answer, index) => {
    ...
    div.onclick = () => selectAnswer(shuffled.indexOf(answer) === q.correct, q);
...
function selectAnswer(isCorrect, question) {
  ...
  options.forEach((opt, i) => {
    if (i === question.correct) opt.classList.add("correct");
      ...
      isCorrect === false &&
      opt.textContent === question.a[question.correct]
    ) {
...
function handleTimeout() {
  ...
  options.forEach((opt) => {
    if (
      opt.textContent ===
      questionsPool[selectedDifficulty][
        Math.floor(Math.random() * questionsPool[selectedDifficulty].length)
      ].a[0]
    ) {
      opt.classList.add("incorrect");
    }
