  ...
  return [
    ...new Set(
      obstacles
        .filter((ob) => {
          if (ob.type === "incoming")
            return ob.y > -200 && ob.y < playerY - 100;
...
function spawnCrashFX(x, y) {
  for (let i = 0; i < 24; i++) {
    ...
    particles.push({
      ...
      r: 4 + Math.random() * 6,
      color: ["#FF3366", "#FF6600", "#FFD700", "#fff"][
...
function drawCoins() {
  ...
  coinObjects.forEach((c) => {
    ...
    ctx.ellipse(
      c.x,
      c.y + bob,
      Math.max(0.5, Math.abs(c.r * spinAmt)),
      c.r,
      0,
      0,
      Math.PI * 2,
    ...
    ctx.ellipse(
      c.x - 3,
      c.y + bob - 3,
      Math.max(0.5, Math.abs(3 * spinAmt)),
      3,
      0,
      0,
      Math.PI * 2,
...
function drawSpeedIndicator() {
  ...
  ctx.fillStyle =
    speedLevel > 5 ? "#FF6644" : speedLevel > 3 ? "#FFD700" : "#00FF88";
...
function drawPowerupTimers() {
  ...
  active.forEach((p, i) => {
    ...
    ctx.textAlign = "left";
    ctx.fillText(
      p.label + " " + Math.ceil(p.timer / 60) + "s",
      W - 78,
      yOff + 12,
    );
    ctx.restore();
...
function activatePowerup(type) {
  ...
  if (type === "shield") {
    shield = true;
    shieldTimer = PUP_DURATION;
  }
  if (type === "magnet") {
    magnet = true;
    magnetTimer = PUP_DURATION;
  }
  if (type === "slow") {
    slowmo = true;
    slowmoTimer = PUP_DURATION;
  }
  if (type === "boost") {
    boost = true;
    boostTimer = PUP_DURATION;
  }
  updatePowerupIcons();
}
...
function updateHUD() {
  ...
  document.getElementById("hudLives").textContent = "❤️".repeat(
    Math.max(0, lives),
  );
}
...
function gameLoop() {
  ...
  const interval = Math.max(
    40,
    OBSTACLE_INTERVAL_BASE - Math.floor(gameSpeed * 6),
  );
  obstacleTimer++;
  ...
  if (coinTimer >= 55) {
    coinTimer = 0;
    if (Math.random() > 0.3) spawnCoinRow();
  }
  powerupTimer++;
  if (powerupTimer >= 280) {
    powerupTimer = 0;
