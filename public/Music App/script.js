...
const songs = Object.entries(categoryCatalog).flatMap(
  ([category, group], categoryIndex) => {
    return group.tracks.map(([title, artist, album], trackIndex) => {
      const audioIndex =
        (categoryIndex * 10 + trackIndex) % audioSources.length;
      const minutes = 4 + ((categoryIndex + trackIndex) % 3);
      const seconds = String(
        (18 + categoryIndex * 7 + trackIndex * 5) % 60,
      ).padStart(2, "0");
...
let likedSongs = new Set(
  JSON.parse(localStorage.getItem(storageKeys.likedSongs) || "[]"),
);
...
function getFilteredSongs() {
  ...
  return songs.filter((song) => {
    ...
    const searchable =
      `${song.title} ${song.artist} ${song.album} ${song.category}`.toLowerCase();
...
function renderSongs() {
  ...
  libraryLabel.textContent =
    activeCategory === "All" ? "All songs" : activeCategory;
  emptyState.classList.toggle("show", filteredSongs.length === 0);

  songGrid.innerHTML = filteredSongs
    .map((song) => {
...
function updateLikeButtons() {
  ...
  playerLikeBtn.setAttribute(
    "aria-label",
    isLiked ? "Remove current song from liked songs" : "Like current song",
  );
}
...
function getNextIndex(direction = 1) {
  ...
  if (isShuffleOn && filteredSongs.length > 1) {
    const availableSongs = filteredSongs.filter(
      (song) => songs.indexOf(song) !== activeIndex,
    );
    return songs.indexOf(
      availableSongs[Math.floor(Math.random() * availableSongs.length)],
    );
  }

  const activeFilteredIndex = filteredSongs.findIndex(
    (song) => songs.indexOf(song) === activeIndex,
  );
  const currentPosition = activeFilteredIndex === -1 ? 0 : activeFilteredIndex;
  const nextPosition =
    (currentPosition + direction + filteredSongs.length) % filteredSongs.length;
...
shuffleBtn.addEventListener("click", () => {
  ...
  shuffleBtn.setAttribute(
    "aria-label",
    isShuffleOn ? "Turn shuffle off" : "Turn shuffle on",
  );
});
...
repeatBtn.addEventListener("click", () => {
  ...
  repeatBtn.setAttribute(
    "aria-label",
    isRepeatOn ? "Turn repeat off" : "Turn repeat on",
  );
});
...
progressInput.addEventListener("change", () => {
  audio.currentTime =
    (Number(progressInput.value) / 100) * (audio.duration || 0);
  isSeeking = false;
});
