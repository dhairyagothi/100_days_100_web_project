...
sepiaBtn.onclick = () =>
  applyFilter(async () => {
    ...
    const sepia = tf.matMul(reshaped, sepiaMatrix).reshape(t.shape).clipByValue(0, 255);
...
sharpenBtn.onclick = () =>
  applyFilter(async () => {
    ...
    const sharpened = tf.depthwiseConv2d(t, kernel, 1, 'same').clipByValue(0, 255).squeeze();
