...
function setupUserInteractionEvents() {
  if (themeToggleBtn)
    themeToggleBtn.addEventListener("click", toggleApplicationTheme);
  ...
  if (btnExport)
    btnExport.addEventListener("click", downloadBackupAsSpreadsheet);
}

function clearFormWarnings() {
  Object.values(errorLogs).forEach((element) => {
...
function checkFormErrors() {
  ...
  if (dateInput && !dateInput.value) {
    ...
    if (selectedDate > today) {
      if (errorLogs.date)
        errorLogs.date.textContent = "Future dates are not allowed.";
      isValid = false;
    }
  }

  if (descriptionInput && !descriptionInput.value.trim()) {
    if (errorLogs.description)
      errorLogs.description.textContent = "Please type a short description.";
    isValid = false;
  }
  ...
  if (checkedType === "Expense" && categorySelect && !categorySelect.value) {
    if (errorLogs.category)
      errorLogs.category.textContent =
        "Please pick a category for this expense.";
    isValid = false;
  }
  return isValid;
}
...
if (transactionForm) {
  transactionForm.addEventListener("submit", (e) => {
    ...
    const checkedTypeNode = document.querySelector(
...
function calculateAndRefreshSummary() {
  ...
  const avgAmount = records.length
    ? records.reduce((sum, item) => sum + item.amount, 0) / records.length
    : 0;
  const savingsPercentage =
    earnedTotal > 0 ? Math.max(0, (currentBalance / earnedTotal) * 100) : 0;
  ...
  if (savingsRateEl)
    savingsRateEl.textContent = `${savingsPercentage.toFixed(1)}%`;
  ...
  Object.entries(categoryTotals).forEach(([category, total]) => {
    ...
    const progressFillBar = document.querySelector(
      `.progress-bar-item[data-category="${category}"] .progress-fill`,
    );
  ...
  const topCategoryPercent =
    spentTotal > 0 ? Math.round((topCategorySum / spentTotal) * 100) : 0;
  if (highestExpensePctEl)
    highestExpensePctEl.textContent = `${topCategoryPercent}%`;
  if (highestExpenseNameEl)
    highestExpenseNameEl.textContent =
      topCategoryLabel === "None" ? "Top Category" : topCategoryLabel;
  if (donutSegment)
    donutSegment.style.strokeDasharray = `${topCategoryPercent} ${100 - topCategoryPercent}`;
  renderHistoryTable();
}
...
function renderHistoryTable() {
  ...
  const chronologicallySorted = [...matchedRecords].sort(
    (a, b) => new Date(b.date) - new Date(a.date),
  );
  transactionsBody.innerHTML = chronologicallySorted
    .map((item) => {
      let readableDate = item.date;
      if (item.date && item.date.includes("-")) {
        const [year, month, day] = item.date.split("-");
        readableDate = new Date(year, month - 1, day).toLocaleDateString(
          "en-IN",
          {
            day: "numeric",
            month: "short",
      ...
      const cashDisplayValue = new Intl.NumberFormat("en-IN", {
        style: "currency",
        currency: "INR",
      }).format(item.amount);
...
function clearAllStoredData() {
  if (
    confirm(
      "Are you completely sure you want to delete all entries? This cannot be undone.",
    )
  ) {
    records = [];
    saveAndReload();
...
function downloadBackupAsSpreadsheet() {
  const currentRecords =
    JSON.parse(localStorage.getItem("budgetbuddy_data")) || [];
  ...
  a.setAttribute(
    "download",
    `ExpenseTracker_Backup_${new Date().toISOString().slice(0, 10)}.csv`,
  );
