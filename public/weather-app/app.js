...
let recentSearches = normalizeRecentSearches(
  JSON.parse(localStorage.getItem('wl_recent') || '[]')
);
let particles = [];
let animFrame;
let sunriseTs = 0,
...
function normalizeRecentSearches(searches) {
  ...
  // preserve the most recent occurrence
  ...
  localStorage.setItem('wl_recent', JSON.stringify(cleaned));
...
async function fetchJSON(url) {
  ...
  if (!res.ok)
    throw new Error(`Network tracking returned status HTTP ${res.status}`);
  return res.json();
}
...
async function fetchByName(query) {
  ...
  try {
    const geo = await fetchJSON(
      `${GEO_BASE}?name=${encodeURIComponent(query)}&count=1&language=en&format=json`
    );
    if (!geo.results || geo.results.length === 0) {
      throw new Error(
        `Could not find "${query}". Check spelling and try again.`
      );
    }
...
async function fetchByCoords(lat, lon, name, country, timezone) {
  ...
  try {
    ...
    const wxUrl =
      `${WX_BASE}?latitude=${lat}&longitude=${lon}` +
    ...
    showError(
      err.message || 'Failed sync cycle to gather remote matrix parameters.'
    );
  }
}
  ...
  navigator.geolocation.getCurrentPosition(
    ...
    (error) => {
      ...
      if (error.code === error.POSITION_UNAVAILABLE)
        systemReason = 'Network location data structural failure.';
      if (error.code === error.TIMEOUT)
        systemReason = 'Location handshake tracking cycle timed out.';

      console.warn(
        `Geolocation sequence warning: ${systemReason}. Loading structural default (London).`
      );
      fetchByName('London');
    },
...
function renderFavourites() {
  ...
  favourites.forEach((city) => {
    ...
    item
      .querySelector('.fav-item-name')
      .addEventListener('click', () => fetchByName(city.split(',')[0].trim()));
...
function animateSunArc() {
  ...
  const pct =
    dayLen > 0 ? Math.max(0, Math.min((now - sunriseTs) / dayLen, 1)) : 0;
  ...
  if (path && dot) {
    ...
    dot.setAttribute(
      'cx',
      (1 - t) * (1 - t) * 10 + 2 * (1 - t) * t * 60 + t * t * 110
    );
    dot.setAttribute(
      'cy',
      (1 - t) * (1 - t) * 55 + 2 * (1 - t) * t * 5 + t * t * 55
    );
  }
}
...
function animateParticles(theme) {
  ...
  particles.forEach((p) => {
    if (theme === 'rainy') {
      ...
      ctx.save();
      ctx.fillStyle = `rgba(253,224,71,${Math.sin(p.life * Math.PI) * 0.5})`;
      ctx.beginPath();
      ctx.arc(p.x, p.y, p.r, 0, Math.PI * 2);
      ctx.fill();
      ctx.restore();
      ...
      p.twinkle += p.speed;
      if (p.twinkle > 1) p.twinkle = 0;
      ctx.save();
      ctx.fillStyle = `rgba(255,255,255,${0.3 + Math.sin(p.twinkle * Math.PI) * 0.5})`;
      ctx.beginPath();
      ctx.arc(p.x, p.y, p.r, 0, Math.PI * 2);
      ctx.fill();
      ctx.restore();
...
function validateInput(str) {
  if (!str) {
    showError('Please enter a target city name first.');
    return false;
  }
  if (/[0-9]/.test(str)) {
    showError('Metric parameters cannot validate strings containing numbers.');
    return false;
  }
  if (/[<>{}[\]\\]/.test(str)) {
    showError('Input contains unauthorized script parameters.');
    return false;
  }
  return true;
}
...
geoBtn.addEventListener('click', () => {
  ...
  navigator.geolocation.getCurrentPosition(
    ...
    () =>
      showError(
        'Location permission denied. Enter city coordinate strings manually.'
      )
  );
});
