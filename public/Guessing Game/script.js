class GuessingGame {
  ...
  setDifficulty(level) {
    ...
    this.newGame();
    this.showMessage(
      `Difficulty changed to ${level.toUpperCase()}! Range: ${this.range.min}-${this.range.max}`,
      "info",
    );
  }
  ...
  newGame() {
    ...
    this.showMessage(
      `🎮 New game started! Guess between ${this.range.min} and ${this.range.max}`,
      "success",
    );
    this.updateHint("");
  ...
  makeGuess() {
    ...
    if (guess < this.range.min || guess > this.range.max) {
      this.showMessage(
        `❌ Please guess between ${this.range.min} and ${this.range.max}!`,
        "error",
      );
      return;
    }
  ...
  handleWin() {
    this.showMessage(
      `🎉 Congratulations! You guessed it in ${this.attempts} attempts! 🎉`,
      "success",
    );
    ...
    if (!this.bestScore || this.attempts < this.bestScore) {
      ...
      this.updateBestScoreDisplay();
      this.showMessage(
        `🏆 New record! Best score: ${this.bestScore} attempts!`,
        "success",
      );
    }
  }
  ...
  handleLoss() {
    this.showMessage(
      `😔 Game Over! The number was ${this.secretNumber}.`,
      "error",
    );
    this.guessInput.disabled = true;
    this.guessBtn.disabled = true;
