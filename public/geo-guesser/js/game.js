...
const Game = {
  ...
  init(difficulty = "medium") {
    ...
    this.state = {
      ...
      locations: GameData.getRandomLocations(
        difficulty,
        GameData.config.totalRounds,
      ),
      currentLocation: null,
      guessCoords: null,
      shownHints: 1,
  ...
  nextRound() {
    ...
    this.state.currentLocation =
      this.state.locations[this.state.currentRound - 1];
  ...
  confirmGuess() {
    ...
    // Apply hint penalty
    const hintPenalty =
      (this.state.shownHints - 1) * GameData.config.hintPenalty;
  ...
  endGame() {
    ...
    const avgDistance = Math.round(
      this.state.distances.reduce((a, b) => a + b, 0) /
