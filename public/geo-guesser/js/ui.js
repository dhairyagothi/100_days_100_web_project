...
const UI = {
  ...
  attachEventListeners() {
    ...
    document.getElementById("startGameBtn").addEventListener("click", () => {
      const difficulty = document.querySelector(".btn-difficulty.active")
        .dataset.difficulty;
  ...
  showResultScreen(data) {
    ...
    document.getElementById("resultIcon").textContent = Utils.getResultIcon(
      data.distance,
    );
    document.getElementById("resultTitle").textContent = Utils.getResultTitle(
      data.distance,
    );
    document.getElementById("distanceValue").textContent =
      Utils.formatNumber(data.distance) + " km";
    document.getElementById("roundScore").textContent = Utils.formatNumber(
      data.score,
    );
    document.getElementById("resultTotalScore").textContent =
      Utils.formatNumber(data.totalScore);
    document.getElementById("actualLocationName").textContent =
      data.locationName;

    // Update header score
    document.getElementById("totalScore").textContent = Utils.formatNumber(
      data.totalScore,
    );
  },
  ...
  showInlineResult(data) {
    ...
    if (resultInfo) {
      ...
      document.getElementById("inlineResultIcon").textContent =
        Utils.getResultIcon(data.distance);
      document.getElementById("inlineResultTitle").textContent =
        Utils.getResultTitle(data.distance);

      document.getElementById("inlineLocationName").textContent =
        data.locationName;
      document.getElementById("inlineDistance").textContent =
        Utils.formatNumber(data.distance) + " km";
      document.getElementById("inlineScore").textContent = Utils.formatNumber(
        data.score,
      );
    }
  ...
  showFinalScreen(data) {
    ...
    // Update final score
    document.getElementById("finalScoreValue").textContent = Utils.formatNumber(
      data.finalScore,
    );

    // Performance message
    const maxPossible =
      GameData.config.scoring.perfect.points * GameData.config.totalRounds;
    ...
    document.getElementById("avgDistance").textContent =
      Utils.formatNumber(data.avgDistance) + " km";
    document.getElementById("bestRound").textContent = Utils.formatNumber(
      data.bestRound,
    );
    document.getElementById("finalHighScore").textContent = Utils.formatNumber(
      data.highScore,
    );
  ...
  displayHints(location, count) {
    ...
    img.onerror = function () {
      ...
      placeholder.style.cssText =
        "color: white; font-size: 3rem; display: flex; align-items: center; justify-content: center; width: 100%; height: 100%;";
  ...
  updateWelcomeStats() {
    ...
    document.getElementById("highScore").textContent = Utils.formatNumber(
