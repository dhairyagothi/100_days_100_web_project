window.onload = function () {
  const userResponse = prompt(
    "Hello! Welcome to the mysterious world. Are you ready for what lies ahead?",
  );
...
function triggerScreenDespair() {
  if (!despairTriggered) {
    ...
    despairOverlay.innerHTML =
      "<h1 class='despair-text'>THE VOID AWAITS...</h1>";
...
function playRandomSound() {
  const audio = new Audio(
    eerieSounds[Math.floor(Math.random() * eerieSounds.length)],
  );
  audio.play();
}
...
card1.addEventListener("mouseenter", () => {
  const randomQuote =
    scaryQuotes[Math.floor(Math.random() * scaryQuotes.length)];
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function showMessage() {
    ...
    if (clickCount <= messages.length) {
      ...
      surpriseMessageDiv.textContent =
        "The potion is now mixed! Something magical may happen...";
...
function randomizeText() {
  const messages = ["Stop clicking.", "It's already too late..."];
...
function triggerCrash() {
  ...
  let chaosInterval = setInterval(() => {
    ...
    chaosElement.style.animation =
      "spin-glitch 0.5s infinite linear, move-glitch 0.5s infinite";
  ...
  setTimeout(() => {
    ...
    document
      .querySelectorAll(".chaos-element, .glitchy-text")
...
function createMemory() {
  for (let i = 0; i < 70; i++) {
    ...
    errorMemoryText.style.top = Math.random() * window.innerHeight + "px";
    errorMemoryText.style.left = Math.random() * window.innerWidth + "px";
    errorMemoryText.style.animation =
      "error-fade 0.5s alternate infinite, rotate-wobble 1s infinite";
  }
