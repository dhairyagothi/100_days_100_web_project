...
function startGame(mode) {
  ...
  difficultyText.textContent = `Difficulty: ${mode[0].toUpperCase()}${mode.slice(1)}`;
...
function showComboText(x, y, comboLevel) {
  const comboTexts = ['2× COMBO!', '3× COMBO!!', 'ON FIRE! 🔥', 'UNSTOPPABLE!', 'LEGENDARY! ⚡'];
  ...
  const colors = ['var(--cyan)', 'var(--magenta)', 'var(--lime)', 'var(--violet)'];
...
function updateGame() {
  ...
  const isCollidingWithPaddle =
    ballY + ballSize >= paddleY && ballX + ballSize >= paddleX && ballX <= paddleX + paddleWidth;
