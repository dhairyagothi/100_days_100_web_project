...
function processFiles(files) {
  ...
  Array.from(files).forEach((file) => {
    ...
    if (file.size > MAX_FILE_SIZE) {
      showToast(
        `Skipped ${file.name} - Exceeds 10MB`,
        "error",
        "ph-warning-circle",
      );
      return;
  ...
  if (addedCount > 0) {
    showToast(
      `Added ${addedCount} images to queue`,
      "success",
      "ph-check-circle",
    );
    updateUI();
...
function getTargetBytes() {
  ...
  if (isNaN(targetKB) || targetKB < 50) {
    showToast(
      "Images cannot be compressed below 50KB safely.",
      "error",
      "ph-warning",
    );
    return null;
    ...
    if (item.status === "compressing" && !item.result) {
      try {
        ...
        if (result.hitUnsafeLimit) {
          showToast(
            `Unable to safely compress ${item.file.name} to target without major loss.`,
            "error",
            "ph-warning-circle",
          );
        }
...
async function recompressAll() {
  ...
  applyTargetBtn.innerHTML =
    '<i class="ph ph-spinner-gap" style="animation: spin 1s linear infinite;"></i> Processing...';
...
function renderQueue() {
  ...
  fileQueue.forEach((item) => {
    ...
    removeBtn.addEventListener("click", () => {
      if (item.result) {
        if (item.result.originalUrl)
          URL.revokeObjectURL(item.result.originalUrl);
        if (item.result.compressedUrl)
          URL.revokeObjectURL(item.result.compressedUrl);
      }
...
downloadAllBtn.addEventListener("click", async () => {
  ...
  downloadAllBtn.innerHTML =
    '<i class="ph ph-spinner-gap" style="animation: spin 1s linear infinite;"></i> Zipping...';
