    ...
    reader.onload = (e) => {
      ...
      img.onload = async () => {
        ...
        if (file.size <= targetSizeBytes) {
          ...
          if (blob) {
            return resolve(
              finalize(file, e.target.result, blob, baseWidth, baseHeight, scaled, false)
            );
          }
        }
        ...
        if (hitTarget && bestBlob) {
          return resolve(
            finalize(file, e.target.result, bestBlob, baseWidth, baseHeight, scaled, false)
          );
        }
        ...
        const fallbackBlob = await generateCanvasBlob(
          img,
          baseWidth,
          baseHeight,
          MIN_QUALITY,
        ...
        if (fallbackBlob && fallbackBlob.size <= targetSizeBytes) {
          return resolve(
            finalize(file, e.target.result, fallbackBlob, baseWidth, baseHeight, scaled, false)
          );
        }
