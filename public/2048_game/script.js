...
const ACHIEVEMENTS = [
  {
    ...
    label: "Quarter Way",
    desc: "Reach 256",
    check: (b) => maxTile(b) >= 256,
  },
  {
    id: "first512",
    label: "Halfway",
    desc: "Reach 512",
    check: (b) => maxTile(b) >= 512,
  },
  {
    id: "first1024",
    label: "So Close",
    desc: "Reach 1024",
    check: (b) => maxTile(b) >= 1024,
  },
  {
    id: "first2048",
    label: "Winner!",
    desc: "Reach 2048",
    check: (b) => maxTile(b) >= 2048,
  },
  {
    id: "moves100",
    label: "Century",
...
function doMove(dir) {
  ...
  for (let r = 0; r < N; r++) {
    ...
    for (let c = 0; c < N; c++)
      if (res.row[c] > tmp[r][c] && res.row[c] > 0)
        mergePositions.push({ r, c, v: res.row[c] });
    tmp[r] = res.row;
    pts += res.pts;
  }
  ...
  if (isLost()) {
    over = true;
    showToast(
      reviveChance > 0 ? `Chance left: ${reviveChance}` : "No chances left",
    );
    if (mode === "timed") clearInterval(timerInterval);
    stats.games++;
    stats.best = Math.max(stats.best, score);
