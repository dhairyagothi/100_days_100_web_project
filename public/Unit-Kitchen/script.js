...
const parseIngredientLine = (line) => {
  ...
  // Normalize compact quantity-unit formats
  line = line.replace(
  ...
  // Support compact quantity+unit formats
  // Examples:
  // 200g sugar
  // 250ml milk
  // 1kg flour
  ...
  rest = rest.replace(
    /^(cups?|cup|tbsp|tsp|oz|lbs?|lb|g|kg|ml|l|pinch|cloves?|large?|medium?|small?)(?=\S)/i,
    '$1 '
  );

  const unitRe =
    /^(cups?|cup|tbsp|tsp|oz|lbs?|lb|g|kg|ml|l|pinch|cloves?|large?|medium?|small?)\b\s*/i;
...
const renderOutput = () => {
  ...
  outputList.innerHTML = parsedIngredients
    .map((ing) => {
      if (ing.amount === null) {
        return `<li>
...
const parseAndRender = () => {
  const lines = ingredientInput.value.split('\n').filter((l) => l.trim());
