...
function calcLayout() {
  ...
  LANE3_Y = ROAD_Y + ROAD_H * 0.8; // oncoming traffic (bottom)
}
...
let bestRun = 0;
try {
  bestRun = parseInt(localStorage.getItem('cbr_best') || '0', 10) || 0;
} catch (e) {
  bestRun = 0;
}
...
function sfxDeliver() {
  ...
  setTimeout(() => playTone(990, 0.15, 'triangle', 0.15), 90);
}
function sfxCrash() {
  playTone(120, 0.35, 'sawtooth', 0.22);
}
function sfxCombo(c) {
  playTone(900 + c * 120, 0.16, 'triangle', 0.16);
}
function sfxPowerup() {
  playTone(1400, 0.18, 'sine', 0.15);
  setTimeout(() => playTone(1800, 0.18, 'sine', 0.12), 100);
}
function sfxHorn() {
  playTone(300, 0.4, 'square', 0.2);
  playTone(252, 0.4, 'square', 0.15);
}
function sfxLowFuel() {
...
function spawnTraffic() {
  ...
  trafficCars.push({
    ...
    w: 55 + Math.random() * 25,
    h: 24,
    color: ['#e74c3c', '#3498db', '#f1c40f', '#9b59b6', '#1abc9c', '#e67e22'][
  ...
  trafficCars.push({
    ...
    w: 55 + Math.random() * 20,
    h: 24,
    color: ['#c0392b', '#2980b9', '#d35400', '#8e44ad'][
...
function genWins(bw, bh) {
  ...
  const cols = Math.floor(bw / 18),
    rows = Math.floor(bh / 22);
  for (let r = 0; r < rows; r++)
    for (let c = 0; c < cols; c++) wins.push({ c, r, on: Math.random() > 0.4 });
  return wins;
}
...
function genTrees() {
  ...
  for (let i = 0; i < 80; i++)
    trees.push({
      worldX: i * 180 + Math.random() * 100,
      type: i % 4 === 0 ? 'lamp' : 'tree',
    });
}
...
function initGame() {
  ...
  powerups = [];
  powerupSpawnTimer = 6 + Math.random() * 6;
  worldX = 0;
  weather = 'clear';
...
function update(dt) {
  ...
  if (!bus.crashed) {
    ...
    const rainPenalty =
      weather === 'heavy_rain' ? 0.7 : weather === 'rain' ? 0.85 : 1;
    ...
    if (bus.speed > topSpd)
      bus.speed = Math.max(topSpd, bus.speed - bus.brake * dt);

    // Vertical lane movement
    if (lt)
      bus.laneTarget = Math.max(
        ROAD_Y + bus.h / 2 + 5,
        bus.laneTarget - 90 * dt
      );
    if (rt)
      bus.laneTarget = Math.min(
        ROAD_Y + ROAD_H - bus.h / 2 - 5,
        bus.laneTarget + 90 * dt
      );
  ...
  if (!bus.crashed) {
    fuelStations.forEach((f) => {
      ...
      if (dist < 70 && bus.speed < 35 && bus.fuel < 100) {
        ...
        if (before < 100 && bus.fuel >= 100)
          showPopup('TANK FULL! ⛽', '#4FC3F7');
      }
    });
  }
    ...
    busStops.forEach((stop) => {
      ...
      if (dist < 80 && bus.speed < 30 && bus.passengers < bus.maxPass) {
        ...
        if (boarding > 0) {
          ...
          sfxPickup();
          showPopup(
            `+₹${boarding * 20} BOARDED!` + (bonus ? ' 📯' : ''),
            '#00E676'
          );
          updatePassIcons();
          // assign destination stop
          if (!bus.destStop) {
            const future = busStops.filter(
              (s2) => s2.worldX > stop.worldX + 200 && !s2.delivered
            );
  ...
  if (!bus.crashed) {
    trafficCars.forEach((c) => {
      ...
      const bx = bus.x,
        by = bus.y;
      if (
        Math.abs(bx - cx) < bus.w / 2 + c.w / 2 - 8 &&
  ...
  if (!bus.crashed && bus.speed > 40) {
    signals.forEach((s) => {
      ...
      const inZone =
        Math.abs(bus.x - sx) < 40 && Math.abs(bus.y - LANE2_Y) < 40;
...
function drawNavigationArrow() {
  ...
  if (targetX < 0 || targetX > W) {
    ...
    ctx.fillText(targetX < 0 ? '⬅' : '➡', arrowX, 100);
...
function drawSky() {
  ...
  if (weather === 'clear') {
    ...
    g.addColorStop(1, '#6a7a8a');
  } else {
    g.addColorStop(0, '#1a1a2e');
    g.addColorStop(1, '#3a3a4e');
  }
  ctx.fillStyle = g;
  ...
  if (weather === 'clear') {
    ...
    // Storm clouds
    ctx.fillStyle =
      weather === 'heavy_rain' ? 'rgba(30,30,50,0.8)' : 'rgba(70,70,90,0.6)';
    for (let i = 0; i < 6; i++) {
...
function drawGround() {
  ...
  if (weather === 'clear') {
    gg.addColorStop(0, '#5a8a4a');
    gg.addColorStop(1, '#3a6a2a');
  } else {
    gg.addColorStop(0, '#3a5a32');
    gg.addColorStop(1, '#2a4a22');
  }
  ctx.fillStyle = gg;
...
function drawTrees() {
  trees.forEach((t) => {
    ...
    if (t.type === 'lamp') {
      ...
      ctx.shadowBlur = 20;
      ctx.beginPath();
      ctx.ellipse(sx, GROUND_Y + 18, 10, 6, 0, 0, Math.PI * 2);
      ctx.fill();
      ctx.shadowBlur = 0;
    } else {
      // Tree
      ...
      ctx.beginPath();
      ctx.arc(sx, GROUND_Y + 10, 22, 0, Math.PI * 2);
      ctx.fill();
      ctx.fillStyle = weather === 'clear' ? '#3a9a3a' : '#2a7a2a';
      ctx.beginPath();
      ctx.arc(sx - 8, GROUND_Y + 17, 15, 0, Math.PI * 2);
      ctx.fill();
    }
  });
}
...
function drawSignals() {
  signals.forEach((s) => {
    ...
    ctx.fillStyle = '#111';
    ctx.beginPath();
    ctx.roundRect(sx - 12, ROAD_Y - 72, 24, 42, 4);
    ctx.fill();
    // Lights
    const lc = { red: '#ff2222', yellow: '#ffcc00', green: '#00ff55' };
    ['red', 'yellow', 'green'].forEach((col, i) => {
      ctx.beginPath();
      ctx.arc(sx, ROAD_Y - 66 + i * 13, 5.5, 0, Math.PI * 2);
      if (
        (s.state === 'red' && col === 'red') ||
        (s.state === 'green' && col === 'green')
      ) {
        ctx.fillStyle = lc[col];
        ctx.shadowColor = lc[col];
        ctx.shadowBlur = 12;
      } else {
...
function drawBusStops() {
  busStops.forEach((stop) => {
    ...
    if (bus.destStop && bus.destStop.id === stop.id) {
      ...
      ctx.beginPath();
      ctx.arc(sx, sy + 14, 32, 0, Math.PI * 2);
      ctx.fill();
...
function drawBus() {
  ...
  if (bus.crashed) {
    ...
    for (let s = 0; s < 8; s++) {
      ...
      ctx.beginPath();
      ctx.arc(bx + Math.cos(a) * r, by + Math.sin(a) * r, 3, 0, Math.PI * 2);
...
function drawBusShape(bx, by, crashed) {
  ...
  ctx.fillStyle = '#000';
  ctx.beginPath();
  ctx.ellipse(bx + 5, by + h * 0.5 + 2, w * 0.45, 7, 0, 0, Math.PI * 2);
  ctx.fill();
  ctx.globalAlpha = 1;
  ...
  for (let wi = 0; wi < 4; wi++)
    ctx.fillRect(x + 8 + wi * 19, y + 4, 14, h * 0.28);
  ...
  [
    ...
    ctx.fillStyle = '#1a1a1a';
    ctx.beginPath();
    ctx.ellipse(x + wx, y + wy - 4, 10, 7, 0, 0, Math.PI * 2);
    ctx.fill();
    ctx.fillStyle = '#555';
    ctx.beginPath();
    ctx.ellipse(x + wx, y + wy - 4, 5, 3.5, 0, 0, Math.PI * 2);
    ctx.fill();
  });
  ...
  ctx.shadowBlur = 15;
  ctx.beginPath();
  ctx.ellipse(x + w + 2, y + h * 0.65, 5, 4, 0, 0, Math.PI * 2);
  ctx.fill();
  ctx.shadowBlur = 0;
  ...
  ctx.shadowBlur = 8;
  ctx.beginPath();
  ctx.ellipse(x + 2, y + h * 0.65, 4, 3.5, 0, 0, Math.PI * 2);
  ctx.fill();
  ctx.shadowBlur = 0;
  ...
  if (bus.passengers > 0) {
    ...
    ctx.beginPath();
    ctx.roundRect(bx - 14, y - 16, 28, 14, 4);
    ctx.fill();
    ctx.fillStyle = '#FFD600';
    ctx.font = 'bold 9px Nunito';
...
function drawCarShape(cx, cy, w, h, color, flip, type) {
  ...
  ctx.fillStyle = '#000';
  ctx.beginPath();
  ctx.ellipse(cx + 4, cy + h * 0.5 + 1, w * 0.42, 5, 0, 0, Math.PI * 2);
  ctx.fill();
  ctx.globalAlpha = 1;
  ...
  if (type === 'truck') {
    ctx.beginPath();
    ctx.roundRect(x + 2, y + 3, w - 4, h * 0.3, 3);
    ctx.fill();
  } else {
    ctx.beginPath();
    ctx.roundRect(x + w * 0.15, y + 4, w * 0.65, h * 0.28, 3);
    ctx.fill();
  }
  ...
  ctx.beginPath();
  ctx.roundRect(x + w * 0.18, y + 6, w * 0.56, h * 0.2, 2);
  ctx.fill();
  ...
  [
    ...
    ctx.fillStyle = '#1a1a1a';
    ctx.beginPath();
    ctx.ellipse(x + wx, y + wy - 3, 8, 5, 0, 0, Math.PI * 2);
    ctx.fill();
    ctx.fillStyle = '#555';
    ctx.beginPath();
    ctx.ellipse(x + wx, y + wy - 3, 4, 2.5, 0, 0, Math.PI * 2);
    ctx.fill();
  });
  ...
  ctx.shadowBlur = 8;
  ctx.beginPath();
  ctx.ellipse(x + w + 1, y + h * 0.68, 4, 3, 0, 0, Math.PI * 2);
  ctx.fill();
  ctx.shadowBlur = 0;
...
function drawRainOverlay() {
  ctx.fillStyle =
    weather === 'heavy_rain' ? 'rgba(50,80,120,0.12)' : 'rgba(60,90,130,0.06)';
...
function updateHUD() {
  ...
  ts.className =
    'hud-val' + (timeLeft < 20 ? ' red' : timeLeft < 40 ? ' yellow' : '');
...
function endGame() {
  ...
  if (score > bestRun) {
    bestRun = score;
    try {
      localStorage.setItem('cbr_best', String(bestRun));
    } catch (e) {}
  }
...
(function idleLoop() {
  if (!gameRunning) {
    ...
    g.addColorStop(0.75, '#555');
    g.addColorStop(0.75, '#5a8a4a');
...
function updateMiniMap() {
  ...
  busEl.style.left =
    Math.min(100, Math.max(0, (worldX / routeLength) * 100)) + '%';

  let target = bus.destStop || busStops.find((s) => !s.pickedUp);
  if (target) {
    targetEl.style.left =
      Math.min(100, Math.max(0, (target.worldX / routeLength) * 100)) + '%';
...
function resetOverlayUI() {
  ...
  title.textContent = '🚌 CITY BUS RUSH';
  ...
  playBtn.textContent = 'START DRIVING!';
