...
const DEFAULTS = {
  ...
  moodReminder: false,
  playlistAlerts: true,
  geminiApiKey: "", // for Hugging Face API (optional, only needed if user wants to use the smart recommendation feature that learns from mood history)
};
...
function populateUI() {
  ...
  if (geminiInput) {
    geminiInput.value = cfg.geminiApiKey || "";
  }
}
...
function checkLocalStorageStatus() {
  try {
    ...
    localStorageStatus.textContent =
      "localStorage is unavailable (private mode or blocked).";
...
function collectConfig() {
  ...
  if (geminiInput) {
    cfg.geminiApiKey = geminiInput.value.trim();
  }
}
...
exportFavBtn.addEventListener("click", () => {
  const favs = loadJSON(KEYS.favorites);
  if (!favs.length) {
    showToast("No favourites to export.", true);
    return;
  }
...
resetRecDataBtn.addEventListener("click", () => {
  openModal(
    ...
    () => {
      try {
        localStorage.removeItem(KEYS.recData);
      } catch {}
      refreshDataStats();
...
saveBtn.addEventListener("click", () => {
  ...
  applyAccent(cfg.accent);
  showToast(
    ok ? "Settings saved!" : "Could not save (storage unavailable).",
    !ok,
  );
