...
try {
  favorites = JSON.parse(localStorage.getItem("moodMusicFavorites")) || [];
} catch {}
...
function pickSurprise() {
  const options = currentMood ? moods.filter((m) => m.id !== currentMood.id) : moods;
    ...
    card.innerHTML = `
      <div class="song-cover-wrap">
        ...
        <div class="song-cover-overlay">
          <a href="${spUrl}" target="_blank" rel="noopener noreferrer" class="overlay-btn overlay-btn-spotify" title="Find on Spotify">
            <i class="fa-brands fa-spotify"></i>
          </a>
          <a href="${ytUrl}" target="_blank" rel="noopener noreferrer" class="overlay-btn overlay-btn-yt" title="Find on YouTube">
      ...
      <div class="song-body">
        ...
        <div class="song-footer">
          ...
          <button class="heart-btn ${isFav ? "active" : ""}" data-id="${song.id}" aria-label="${isFav ? "Remove from liked" : "Add to liked"}">
...
function toggleFav(songId, btn) {
  ...
  try {
    localStorage.setItem("moodMusicFavorites", JSON.stringify(favorites));
  } catch {}
  updateFavBadge();
}
...
const detectorCloseBtn = document.getElementById("detectorCloseBtn");
const detectorPrivacy = document.getElementById("detectorPrivacy");
const startCameraBtn = document.getElementById("startCameraBtn");
const detectorVideoWrap = document.getElementById("detectorVideoWrap");
const detectorVideo = document.getElementById("detectorVideo");
const detectNowBtn = document.getElementById("confirmMoodBtn");
console.log(
  "Detect button:",
  detectNowBtn
);
...
detectMoodBtn.addEventListener("click", () => {
  detectorOverlay.classList.add("active");
  detectorPrivacy.style.display = "flex";
});

function closeDetector() {
  detectorOverlay.classList.remove("active");
  if (detectorStream) {
    detectorStream.getTracks().forEach((t) => t.stop());
    detectorStream = null;
  }
  detectorVideo.srcObject = null;
  const liveLabel = document.getElementById("detectorLiveLabel");
  if (liveLabel) liveLabel.remove();
}

detectorCloseBtn.addEventListener("click", closeDetector);

detectorOverlay.addEventListener("click", (e) => {
  if (e.target === detectorOverlay) closeDetector();
});
...
startCameraBtn.addEventListener("click", async () => {
  try {
    ...
    detectorVideo.srcObject =
      detectorStream;

    await detectorVideo.play();

    await new Promise((resolve) => {
      detectorVideo.onloadedmetadata =
        resolve;
    });

    detectorPrivacy.style.display =
      "none";

    detectorVideoWrap.style.display =
      "block";
...
function captureBase64FromVideo() {
  ...
  canvas.width = detectorVideo.videoWidth;
  canvas.height = detectorVideo.videoHeight;
  canvas.getContext("2d").drawImage(detectorVideo, 0, 0, canvas.width, canvas.height);
  const dataUrl = canvas.toDataURL("image/jpeg");
  return dataUrl.split(",")[1];
}

async function detectMoodWithGemini(base64) {
  try {
    const settings =
      JSON.parse(
        localStorage.getItem("moodMusic_settings")
      ) || {};
