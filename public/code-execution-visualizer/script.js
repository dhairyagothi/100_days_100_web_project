const PRESETS = {
  ...
  arrays: {
    label: "Arrays",
    code: `int arr[] = {4, 8, 15, 16, 23, 42};\nint target = arr[3];\nint doubled = target * 2;\narr[0] = doubled;\nint mix = arr[0] + arr[5];`,
  },
  bubbleSort: {
    label: "Bubble Sort",
    code: `int arr[] = {7, 3, 9, 2, 5};\nfor (int i = 0; i < 4; i++) {\n  for (int j = 0; j < 4 - i; j++) {\n    if (arr[j] > arr[j + 1]) {\n      int temp = arr[j];\n      arr[j] = arr[j + 1];\n      arr[j + 1] = temp;\n    }\n  }\n}`,
  },
  binarySearch: {
    label: "Binary Search",
    code: `int arr[] = {2, 4, 7, 11, 18, 23, 31};\nint target = 18;\nint low = 0;\nint high = 6;\nwhile (low <= high) {\n  int mid = (low + high) / 2;\n  if (arr[mid] == target) {\n    int foundAt = mid;\n    break;\n  }\n  if (arr[mid] < target) {\n    low = mid + 1;\n  }\n  if (arr[mid] > target) {\n    high = mid - 1;\n  }\n}`,
  },
  recursion: {
    label: "Recursion",
...
function setupEvents() {
  ...
  ui.presetSelect.addEventListener(
    "change",
    (e) => (state.selectedPreset = e.target.value),
  );
  ...
  document.addEventListener("keydown", (e) => {
    if (e.target === ui.codeInput && !e.ctrlKey && !e.metaKey && e.key !== " ")
      return;
    if (e.code === "Space") {
      e.preventDefault();
      runPauseToggle();
...
function runSimulation() {
  if (!state.steps.length || state.pointer >= state.steps.length)
    compileProgram();
...
function pauseSimulation() {
  ...
  ui.runStatus.textContent =
    state.pointer >= state.steps.length && state.steps.length
      ? "Completed"
      : "Paused";
}
...
function resetPlayback(silent) {
  ...
  ui.whatHappened.innerHTML =
    "<p>Each step will include a human-readable explanation.</p>";
  setExecutionLine(-1);
  enqueueRender();
...
function compileProgram() {
  ...
  if (/\bfib\s*\(/.test(code) || state.selectedPreset === "recursion")
    compileRecursion(code);
  else if (
    /while\s*\(.*<=.*\)/.test(code) ||
    state.selectedPreset === "binarySearch"
  )
    compileBinarySearch(code);
  else compileInterpreter(code);

  ui.runStatus.textContent = state.steps.length ? "Ready" : "Idle";
  log(
    state.steps.length
      ? `Compiled ${state.steps.length} steps.`
...
function compileInterpreter(code) {
  ...
  estimateComplexity(
    ...
    null,
    null,
    "Loop and branch analysis from parsed structure.",
  );
...
function compileBinarySearch(code) {
  ...
  const arr = (arrLine.text.match(/\{(.*)\}/)?.[1] || "")
    .split(",")
    .map((x) => Number(x.trim()))
  ...
  pushCompiledStep(compileState, {
    line: arrLine.line,
    code: arrLine.raw,
    action: `Declared sorted array of ${arr.length} elements`,
    explain: "Array is sorted, enabling binary elimination.",
  });
  pushCompiledStep(compileState, {
    line: targetLine.line,
    code: targetLine.raw,
    action: `Target set to ${target}`,
    explain: "Search value is loaded into memory.",
  });

  while (low <= high) {
    const mid = Math.floor((low + high) / 2);
    compileState.memory = { ...compileState.memory, low, high, mid, target };
    compileState.loops.push({
      line: targetLine.line,
      iteration: compileState.loops.length + 1,
    ...
    if (arr[mid] === target) {
      ...
      pushCompiledStep(compileState, {
        ...
        explain: "Match found, loop exits.",
      });
      break;
    }
    if (arr[mid] < target) {
      ...
      pushCompiledStep(compileState, {
        ...
        code: "low = mid + 1",
        action: `Move low -> ${low}`,
        explain: "Discard left half including mid.",
      });
    } else {
      high = mid - 1;
      pushCompiledStep(compileState, {
        line: targetLine.line,
        code: "high = mid - 1",
        action: `Move high -> ${high}`,
        explain: "Discard right half including mid.",
      });
    }
  }
  estimateComplexity(
...
function compileRecursion(code) {
  ...
  function fib(x, depth, parentId = null) {
    ...
    compileState.recursionNodes.push({
      id: nodeId,
      parentId,
      text: `fib(${x})`,
      depth,
    ...
    pushCompiledStep(compileState, {
      line: callLine?.line || 1,
      code: `fib(${x})`,
      action: `Push fib(${x})`,
      explain: "Function call adds a new frame to stack.",
    });

    if (x <= 1) {
      compileState.stack.pop();
      const node = compileState.recursionNodes.find((t) => t.id === nodeId);
      if (node) node.active = false;
      pushCompiledStep(compileState, {
        line: callLine?.line || 1,
        code: `return ${x}`,
        action: `Return ${x}`,
        explain: "Base case reached, recursion unwinds.",
      });
      return x;
    }
    ...
    pushCompiledStep(compileState, {
      line: callLine?.line || 1,
      code: `return fib(${x - 1}) + fib(${x - 2})`,
      action: `fib(${x}) = ${result}`,
      explain: "Two child results are combined into parent value.",
      memoryPatch: { [`fib_${x}`]: result },
    });
    return result;
  }
  ...
  pushCompiledStep(compileState, {
    line: callLine?.line || 1,
    code: `int answer = fib(${n});`,
    action: `Final answer ${answer}`,
    explain: "Root call returns final computed Fibonacci value.",
    memoryPatch: { answer },
  });
  estimateComplexity(
    [],
    ...
    "O(2^n)",
    "O(n)",
    "Naive recursion forms a binary call tree.",
  );
}
...
function walkBlock(block, compileState, scope) {
  for (const node of block) {
    if (node.type === "line")
      runMini(node.text, node.line, node.raw, compileState, scope);
...
function runIf(node, cs, scope) {
  ...
  pushCompiledStep(cs, {
    line: node.line,
    code: node.raw,
    action: `IF ${node.cond} -> ${result}`,
    explain: "Condition decides whether branch executes.",
  });
  if (!result) return;
  ...
  pushCompiledStep(cs, {
    line: node.line,
    code: node.raw,
    action: "Enter IF frame",
    explain: "Control enters branch scope.",
  });
  ...
  pushCompiledStep(cs, {
    line: node.line,
    code: node.raw,
    action: "Exit IF frame",
    explain: "Branch scope ends and stack frame pops.",
  });
}
function runFor(node, cs, scope) {
  ...
  while (Boolean(evalExpr(node.cond, scope, cs)) && iter < 300) {
    ...
    pushCompiledStep(cs, {
      line: node.line,
      code: node.raw,
      action: `Loop iteration ${iter}`,
      explain: "Loop condition stayed true for another cycle.",
    });
...
function runMini(rawStmt, line, code, cs, scope) {
  ...
  if (arrDecl) {
    ...
    const arr = arrDecl[2]
      .split(",")
      .map((x) => Number(evalExpr(x.trim(), scope, cs)));
    cs.arrays[name] = arr;
    pushCompiledStep(cs, {
      line,
      code,
      action: `Declare ${name}[${arr.length}]`,
      explain: "Contiguous array memory allocated.",
      arrayPatch: { [name]: [...arr] },
    });
    return;
  }
  ...
  if (decl) {
    ...
    scope[name] = value;
    cs.memory[name] = value;
    pushCompiledStep(cs, {
      line,
      code,
      action: `Declare ${name} = ${value}`,
      explain: "A new scalar variable is allocated in local memory.",
      memoryPatch: { [name]: value },
    });
    return;
  }
  ...
  if (/^\w+\+\+$/.test(stmt)) {
    ...
    const value = Number(scope[name] ?? 0) + 1;
    scope[name] = value;
    cs.memory[name] = value;
    pushCompiledStep(cs, {
      line,
      code,
      action: `${name}++ -> ${value}`,
      explain: "Post-increment updates variable by one.",
      memoryPatch: { [name]: value },
    });
    return;
  }
  ...
  if (assign) {
    ...
    if (arrSet) {
      ...
      pushCompiledStep(cs, {
        line,
        code,
        action: `${arrName}[${idx}] = ${value}`,
        explain: "Array cell updated; visual bars and memory map refresh.",
        arrayPatch: { [arrName]: [...cs.arrays[arrName]] },
        sortFocus: {
          arrayName: arrName,
          activeIndices: [idx],
          snapshot: [...cs.arrays[arrName]],
        },
      });
      return;
    }
    scope[left] = value;
    cs.memory[left] = value;
    pushCompiledStep(cs, {
      line,
      code,
      action: `${left} = ${value}`,
      explain: "Existing variable receives a new computed value.",
      memoryPatch: { [left]: value },
    });
    return;
  }
  ...
  pushCompiledStep(cs, {
    line,
    code,
    action: `Skipped unsupported: ${stmt}`,
    explain: "This statement is outside the educational parser subset.",
  });
}
...
function evalExpr(expr, scope, cs) {
  ...
  processed = processed.replace(
    /([a-zA-Z_]\w*)\[(.*?)\]/g,
    (_, name, idxExpr) =>
      cs.arrays[name]?.[Number(evalExpr(idxExpr, scope, cs))] ?? 0,
  );
  processed = processed.replace(/[a-zA-Z_]\w*/g, (token) => {
    ...
    if (Object.prototype.hasOwnProperty.call(cs.memory, token))
      return cs.memory[token];
    return 0;
  });
  try {
...
function pushCompiledStep(cs, step) {
  ...
  if (step.arrayPatch)
    Object.entries(step.arrayPatch).forEach(([k, v]) => {
      cs.arrays[k] = [...v];
    });
...
function estimateComplexity(
  ...
  if (forcedTime) {
    state.complexity = {
      time: forcedTime,
      space: forcedSpace || "O(1)",
      explain: forcedExplain || "Preset complexity model.",
      notes: ["Educational estimate"],
    };
    return;
  }
  let depth = 0;
  const walk = (nodes, d) =>
    nodes.forEach((n) => {
      ...
      if (n.type === "if") walk(n.body, d);
    });
  walk(block, 0);
  let time = "O(1)";
  if (depth === 1) time = "O(n)";
  if (depth === 2) time = "O(n^2)";
  if (depth > 2) time = `O(n^${depth})`;
  if (recursion) time = "O(2^n)";
  state.complexity = {
    time,
    space: recursion ? "O(n)" : "O(1)",
    explain: recursion
      ? "Recursive depth grows call stack linearly while branching grows calls exponentially."
      : "Estimated from nested loop depth and branching.",
    notes: [`Loop depth: ${depth}`],
  };
}
...
function renderSteps() {
  ui.stepList.innerHTML =
    state.steps
      .map(
        (s, i) =>
          `<li class="${i === state.pointer - 1 ? "active" : ""}">#${i + 1} [L${s.line}] ${escapeHtml(s.action)}<div class="step-note">${escapeHtml(s.explain || "")}</div></li>`,
      )
...
function renderMemory() {
  ui.memoryView.innerHTML =
    Object.entries(state.display.memory)
      .map(
        ([k, v]) =>
          `<div class="memory-cell flash"><strong>${escapeHtml(k)}</strong><p>${escapeHtml(v)}</p></div>`,
      )
      .join("") || "<p>No scalar variables yet.</p>";
}
function renderStack() {
  ui.stackView.innerHTML =
    state.display.stack
      .slice()
      .reverse()
      .map(
        (f) =>
          `<div class="stack-frame flash"><strong>${escapeHtml(f.name)}</strong><p>${escapeHtml(f.details)}</p></div>`,
      )
      .join("") || "<p>Stack is empty.</p>";
}
function renderLoops() {
  ui.loopTimeline.innerHTML =
    state.display.loops
      .slice(-24)
      .map(
        (l) =>
          `<div class="timeline-event">L${l.line} � Iter ${l.iteration} <small>${escapeHtml(l.label || "")}</small></div>`,
      )
      .join("") || "<p>No loop events yet.</p>";
}
function renderArrays() {
  const arrays = Object.entries(state.display.arrays);
  if (!arrays.length) {
    ui.arrayView.innerHTML = "<p>No arrays declared.</p>";
    return;
  }
  ui.arrayView.innerHTML = arrays
    .map(
      ([name, arr]) =>
        `<div class="memory-cell"><strong>${escapeHtml(name)}</strong><div class="array-view">${arr.map((v, i) => `<div class="array-item">${i}:${v ?? 0}</div>`).join("")}</div></div>`,
    )
    .join("");
}
function renderSorting() {
  const focus = state.display.sortFocus;
  const first = Object.keys(state.display.arrays)[0];
  if (!first) {
    ui.sortBars.innerHTML = "<p>Bars appear when arrays are manipulated.</p>";
    return;
  }
  const arr = state.display.arrays[first];
  const max = Math.max(...arr, 1);
  ui.sortBars.innerHTML = arr
    .map(
      (v, i) =>
        `<div class="bar-wrap"><div class="bar ${focus?.activeIndices?.includes(i) ? "active" : ""}" style="height:${Math.max(14, Math.round((v / max) * 120))}px"></div><span>${v}</span></div>`,
    )
    .join("");
}
function renderRecursionTree() {
  ...
  if (!nodes.length) {
    ui.recursionTree.innerHTML =
      "<p>Recursion nodes appear for recursive presets.</p>";
    return;
  }
  ui.recursionTree.innerHTML = nodes
    .map(
      (n) =>
        `<div class="tree-node" style="margin-left:${n.depth * 14}px; border-left: 3px solid ${n.active ? "var(--blue)" : "var(--border)"};">${escapeHtml(n.text)}</div>`,
    )
    .join("");
}
function renderComplexity() {
  if (!state.complexity) {
    ui.complexityView.innerHTML = "<p>Compile to estimate complexity.</p>";
    return;
  }
...
function syncEditorDecorations() {
  const code = ui.codeInput.value;
  ui.lineNumbers.innerHTML = code
    .split("\n")
    .map((_, i) => `<div>${i + 1}</div>`)
    .join("");
  ui.syntaxView.innerHTML = code
    .split("\n")
...
function setExecutionLine(lineNumber) {
  if (lineNumber < 1) {
    ui.executionLine.classList.remove("live");
    return;
  }
...
function syntaxHighlight(line) {
  return escapeHtml(line)
    ...
    .replace(
      /\b(int|for|if|while|return|break)\b/g,
      '<span class="t-key">$1</span>',
    )
    .replace(
      /\b(fib|main|arr|sum|low|high|mid|target)\b/g,
...
function setupParticles() {
  ...
  function draw() {
    ...
    for (let i = 0; i < particles.length; i += 1) {
      ...
      if (md < settings.radius) {
        p.vx += (mdx / settings.radius) * 0.004;
        p.vy += (mdy / settings.radius) * 0.004;
      }
      p.x += p.vx;
      p.y += p.vy;
      p.vx *= 0.99;
      ...
      ctx.fillStyle = `hsla(${p.hue},100%,72%,0.72)`;
      ctx.fill();
