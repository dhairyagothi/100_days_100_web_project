...
function setupEvents() {
  ...
  ui.focusModeBtn.addEventListener('click', () => {
    ...
    toast(
      document.body.classList.contains('focus-mode') ? 'Focus mode enabled' : 'Focus mode disabled'
    );
  });
...
function pauseSimulation() {
  ...
  ui.runStatus.textContent =
    state.pointer >= state.steps.length && state.steps.length ? 'Completed' : 'Paused';
}
...
function compileProgram() {
  ...
  else if (/while\s*\(.*<=.*\)/.test(code) || state.selectedPreset === 'binarySearch')
...
function evalExpr(expr, scope, cs) {
  ...
  processed = processed.replace(
    /([a-zA-Z_]\w*)\[(.*?)\]/g,
    (_, name, idxExpr) => cs.arrays[name]?.[Number(evalExpr(idxExpr, scope, cs))] ?? 0
  );
...
function renderRecursionTree() {
  ...
  if (!nodes.length) {
    ui.recursionTree.innerHTML = '<p>Recursion nodes appear for recursive presets.</p>';
    return;
  }
