...
function loadPreferences() {
  ...
  if (savedFields) {
    const checkboxes = document.querySelectorAll(
      'input[name="fields"]:not(:disabled)',
    );
...
function generateFakeData(count, fields) {
  ...
  for (let i = 1; i <= count; i++) {
    ...
    if (fields.includes("username")) {
      record.username =
        baseName.toLowerCase().replace(" ", "") + getRandomInt(10, 999);
    }
    ...
    if (fields.includes("address"))
      record.address = getRandomItem(fakeData.addresses);
    if (fields.includes("company"))
      record.company = getRandomItem(fakeData.companies);
    if (fields.includes("age")) record.age = getRandomInt(18, 60);
    if (fields.includes("gender"))
      record.gender = getRandomItem(fakeData.genders);
