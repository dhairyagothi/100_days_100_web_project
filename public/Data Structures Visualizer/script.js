...
const stackDefinition =
  "A stack is a linear data structure that follows the Last In, First Out (LIFO) principle. Elements are inserted and removed only from the top of the stack.";
...
const stackLearnDefinitions = {
  ...
  overflow:
    "Overflow occurs when a new element is pushed into a completely full stack.",
  underflow:
    "Underflow occurs when a pop operation is attempted on an empty stack.",
...
const queueDefinition =
  "A queue is a linear data structure that follows the First In, First Out (FIFO) principle. Elements are inserted from the rear and removed from the front.";

const queueLearnDefinitions = {
  queue:
    "A queue stores elements in order so the first element added is the first one removed.",
  ...
  overflow:
    "Overflow occurs when an enqueue operation is attempted on a completely full queue.",
  underflow:
    "Underflow occurs when an operation tries to remove or read from an empty queue.",
...
const educationalInfo = {
  array:
    "An array is a linear data structure that stores elements in contiguous memory locations. Each value can be accessed quickly using its index, making arrays useful for organizing collections of data.",

  traversal:
    "Traversal means visiting each element of an array one by one to display, inspect, or process the stored values in a clear sequence.",

  search:
    "Searching is the process of finding a specific element inside an array. Linear search checks elements one at a time until the target is found or the array ends.",
...
const arrayDefinition =
  "An array is a linear data structure that stores elements in indexed positions. It makes values easy to access, update, search, and sort.";
...
const stackVisualizationPanel = document.querySelector(
...
const queueVisualizationPanel = document.querySelector(
...
const initializeArray = () => {
  state.arrayData = Array.from({ length: state.arraySize }, () =>
    getRandomInt(1, 99),
  );
  renderArray();
  renderInputFields();
...
const handleSizeChange = (e) => {
  ...
  if (newSize > oldSize) {
    const newElements = Array.from({ length: newSize - oldSize }, () =>
      getRandomInt(1, 99),
    );
...
const performSearch = async () => {
  ...
  for (let i = 0; i < state.arrayData.length; i++) {
    const box = document.querySelector(`[data-index="${i}"]`);
    if (state.arrayData[i] === null || state.arrayData[i] === undefined)
      continue;
...
const handleSort = async () => {
  ...
  for (let i = 0; i < n - 1; i++) {
    ...
    for (let j = 0; j < n - i - 1; j++) {
      ...
      setOperationStatus(
        `Pass ${passCount}: Comparing ${arr[j]} and ${arr[j + 1]}`,
      );
      await sleep(700);
      ...
      if (arr[j] > arr[j + 1]) {
        ...
        swapCount++;
        setOperationStatus(
          `Pass ${passCount}: Swapping ${arr[j]} and ${arr[j + 1]}`,
        );
    ...
    const sortedBox = document.querySelector(
      `.array-box[data-index="${n - 1 - i}"]`,
    );
    if (sortedBox) {
  ...
  setOperationStatus(
    `Sorting complete! ${swapCount} swaps in ${passCount} passes`,
  );
...
const updateStackSizeDisplay = () => {
  ...
  stackSizeDisplay.classList.toggle(
    "full",
    state.stackData.length === STACK_MAX_SIZE,
  );
...
const showStackWarning = (message) => {
  ...
  stackWarningLabel.textContent = message.includes("Overflow")
    ? "STACK OVERFLOW"
    : "STACK UNDERFLOW";
  ...
  queueItems.querySelectorAll(".queue-item").forEach((item) => {
    ...
    requestAnimationFrame(() => {
      item.style.transition =
        "transform 0.58s cubic-bezier(0.22, 1, 0.36, 1), box-shadow 0.3s ease, border-color 0.3s ease";
...
const updateQueueSizeDisplay = () => {
  ...
  queueSizeDisplay.classList.toggle(
    "full",
    state.queueData.length === QUEUE_MAX_SIZE,
  );
...
const showQueueWarning = (message) => {
  ...
  queueWarningLabel.textContent = message.includes("Overflow")
    ? "QUEUE OVERFLOW"
    : "QUEUE UNDERFLOW";
...
const handleDequeue = async () => {
  ...
  setQueueStatus(
    `${frontValue} deleted from the front of the queue`,
    "success",
  );
  state.isQueueRunning = false;
  setQueueControlsDisabled(false);
...
const highlightQueueEnd = async (operation) => {
  ...
  const item = queueItems.querySelector(
    `.queue-item[data-queue-index="${index}"]`,
  );
  ...
  setQueueStatus(
    `${label} element is: ${state.queueData[index].value}`,
    "success",
  );
...
const switchStructureView = (structure) => {
  const selectedSection = document.querySelector(
    `.visualizer-section[data-structure="${structure}"]`,
  );
  const selectedButton = document.querySelector(
    `.selector-btn[data-structure="${structure}"]`,
  );
...
const typeText = async (
  ...
  text,
  speed = 30,
  typingToken = state.infoTypingToken,
) => {
  element.textContent = "";
...
// -- Definitions --
const llDefinition =
  "A linked list is a linear data structure where elements are stored in nodes connected using pointers. It allows dynamic memory allocation and efficient insertions or deletions.";

const llLearnDefinitions = {
  linkedlist:
    "A linked list stores elements in nodes, where each node holds data and a pointer to the next node in the chain.",
  ...
  insertHead:
    "Inserting at the head prepends a new node. The new node's next pointer links to the old head, and head is updated.",
  insertTail:
    "Inserting at the tail appends a new node. The old tail's next pointer links to the new node, which then points to null.",
  insertIndex:
    "Inserting at an index traverses the list node by node to the target position, then links the new node between existing nodes.",
  deleteHead:
    "Deleting at the head removes the first node. The head pointer is updated to point to the second node, or null if the list becomes empty.",
  deleteTail:
    "Deleting at the tail removes the last node. The second-to-last node's next pointer is updated to null.",
  deleteIndex:
    "Deleting at an index traverses to find the target node, then removes it by reconnecting the previous node to the next node.",
  pointer:
    "A pointer (next) stores the reference to the next node, creating the chain-like linked structure that gives the list its name.",
};
...
const getLLMaxNodes = () =>
  window.innerWidth <= 480 ? LL_MAX_MOBILE : LL_MAX_DESKTOP;
...
const renderLinkedList = (highlightIds = [], newNodeId = null) => {
  ...
  if (llNodeCount) {
    ...
    llNodeCount.classList.toggle(
      "ll-count-full",
      llState.nodes.length >= maxNodes,
    );
  ...
  llState.nodes.forEach((node, index) => {
    ...
    if (isFirst && isLast) {
      ptrRow.innerHTML =
        '<span class="ll-ptr-badge ll-head-badge">HEAD / TAIL</span>';
    } else if (isFirst) {
...
const triggerLLErrorGlow = () => {
  ...
  setTimeout(
    () => llVisualizationPanel.classList.remove("ll-error-state"),
    900,
  );
};
...
const handleLLInsertHead = async () => {
  ...
  if (llState.nodes.length >= maxNodes) {
    triggerLLErrorGlow();
    setLLStatus(
      `Maximum node limit reached for smooth visualization (${maxNodes} nodes max)`,
      "failure",
    );
    resetLLInvalidInputs();
    return;
...
const handleLLInsertTail = async () => {
  ...
  if (llState.nodes.length > 0) {
    ...
    setLLStatus(
      `Linking from current tail (${llState.nodes[llState.nodes.length - 1].value})...`,
      "checking",
    );
    await sleep(520);
  }
...
const handleLLInsertAtIndex = async () => {
  ...
  if (
    indexRaw === "" ||
    isNaN(Number(indexRaw)) ||
    !Number.isInteger(Number(indexRaw))
  ) {
  ...
  if (
    ...
    !Number.isInteger(Number(valueRaw))
  ) {
...
const handleLLDeleteHead = async () => {
  ...
  setLLStatus(
    llState.nodes.length > 0
      ? "Node deleted from head"
      : "Node deleted from head — list is now empty",
    "success",
  );
  llState.isRunning = false;
  setLLControlsDisabled(false);
...
const handleLLDeleteTail = async () => {
  ...
  setLLStatus(
    llState.nodes.length > 0
      ...
      : "Node deleted from tail — list is now empty",
    "success",
  );
  llState.isRunning = false;
  setLLControlsDisabled(false);
...
const handleLLDeleteAtIndex = async () => {
  ...
  if (
    ...
    showLLFieldError(
      llDeleteIndexError,
      "Please enter a valid index (integer ≥ 0)",
    );
    triggerLLErrorGlow();
    setLLStatus("Invalid index — please enter a valid integer", "failure");
    resetLLInvalidInputs();
  ...
  if (index < 0 || index >= llState.nodes.length) {
    ...
    showLLFieldError(
      llDeleteIndexError,
      `Invalid index — valid range is 0 to ${maxIndex}`,
    );
    triggerLLErrorGlow();
  ...
  if (index === 0) {
    ...
    await sleep(320);
    setLLStatus(
      `Node deleted at index 0 (head)${llState.nodes.length === 0 ? " — list is now empty" : ""}`,
      "success",
    );
    llState.isRunning = false;
    setLLControlsDisabled(false);
  ...
  setLLStatus(
    `Found target node at index ${index} (value: ${llState.nodes[index].value}) — deleting...`,
    "checking",
  );
  await sleep(420);
  ...
  setLLStatus(
    `Node deleted at index ${index}${llState.nodes.length === 0 ? " — list is now empty" : ""}`,
    "success",
  );
  llState.isRunning = false;
  setLLControlsDisabled(false);
...
const setupLLEventListeners = () => {
  ...
  llHeadInput.addEventListener("keypress", (e) => {
    if (e.key === "Enter") handleLLInsertHead();
  });
  llTailInput.addEventListener("keypress", (e) => {
    if (e.key === "Enter") handleLLInsertTail();
  });
  llIndexValueInput.addEventListener("keypress", (e) => {
    if (e.key === "Enter") handleLLInsertAtIndex();
  });
  llDeleteIndexInput.addEventListener("keypress", (e) => {
    if (e.key === "Enter") handleLLDeleteAtIndex();
  });

  llLearnBtns.forEach((btn) =>
    btn.addEventListener("click", handleLLLearnClick),
  );
};
