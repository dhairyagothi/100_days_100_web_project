...
const KB_ROWS = [
  ...
  [
    ...
    { l: '',       k: '',             c: 'key-fn-row', style: 'min-width:20px;background:none;border:none' },
    { l: 'F1',  k: 'F1',  c: 'key-fn-row' }, { l: 'F2',  k: 'F2',  c: 'key-fn-row' },
    { l: 'F3',  k: 'F3',  c: 'key-fn-row' }, { l: 'F4',  k: 'F4',  c: 'key-fn-row' },
    { l: '',       k: '',             c: 'key-fn-row', style: 'min-width:12px;background:none;border:none' },
    { l: 'F5',  k: 'F5',  c: 'key-fn-row' }, { l: 'F6',  k: 'F6',  c: 'key-fn-row' },
    { l: 'F7',  k: 'F7',  c: 'key-fn-row' }, { l: 'F8',  k: 'F8',  c: 'key-fn-row' },
    { l: '',       k: '',             c: 'key-fn-row', style: 'min-width:12px;background:none;border:none' },
    { l: 'F9',  k: 'F9',  c: 'key-fn-row' }, { l: 'F10', k: 'F10', c: 'key-fn-row' },
    { l: 'F11', k: 'F11', c: 'key-fn-row' }, { l: 'F12', k: 'F12', c: 'key-fn-row' },
  ...
  // Number row
  [
    { l: '`~', k: 'Backquote' }, { l: '1!', k: 'Digit1' }, { l: '2@', k: 'Digit2' },
    { l: '3#', k: 'Digit3'   }, { l: '4$', k: 'Digit4' }, { l: '5%', k: 'Digit5' },
    { l: '6^', k: 'Digit6'   }, { l: '7&', k: 'Digit7' }, { l: '8*', k: 'Digit8' },
    { l: '9(', k: 'Digit9'   }, { l: '0)', k: 'Digit0' }, { l: '-_', k: 'Minus'  },
    { l: '=+', k: 'Equal'    }, { l: 'Backspace', k: 'Backspace', c: 'key-bs' },
  ],
  // QWERTY row
  [
    { l: 'Tab', k: 'Tab', c: 'key-tab' },
    { l: 'Q', k: 'KeyQ' }, { l: 'W', k: 'KeyW' }, { l: 'E', k: 'KeyE' }, { l: 'R', k: 'KeyR' },
    { l: 'T', k: 'KeyT' }, { l: 'Y', k: 'KeyY' }, { l: 'U', k: 'KeyU' }, { l: 'I', k: 'KeyI' },
    { l: 'O', k: 'KeyO' }, { l: 'P', k: 'KeyP' },
    { l: '[{', k: 'BracketLeft' }, { l: ']}', k: 'BracketRight' }, { l: '|\\', k: 'Backslash' },
  ],
  // ASDF row
  [
    { l: 'Caps Lock', k: 'CapsLock', c: 'key-caps' },
    { l: 'A', k: 'KeyA' }, { l: 'S', k: 'KeyS' }, { l: 'D', k: 'KeyD' }, { l: 'F', k: 'KeyF' },
    { l: 'G', k: 'KeyG' }, { l: 'H', k: 'KeyH' }, { l: 'J', k: 'KeyJ' }, { l: 'K', k: 'KeyK' },
    { l: 'L', k: 'KeyL' }, { l: ';:', k: 'Semicolon' }, { l: '\'"', k: 'Quote' },
    { l: 'Enter', k: 'Enter', c: 'key-enter' },
  ],
  // ZXCV row
  [
    { l: 'Shift', k: 'ShiftLeft', c: 'key-lshift' },
    { l: 'Z', k: 'KeyZ' }, { l: 'X', k: 'KeyX' }, { l: 'C', k: 'KeyC' }, { l: 'V', k: 'KeyV' },
    { l: 'B', k: 'KeyB' }, { l: 'N', k: 'KeyN' }, { l: 'M', k: 'KeyM' },
    { l: ',<', k: 'Comma' }, { l: '.>', k: 'Period' }, { l: '/?', k: 'Slash' },
...
const IGNORED_KEYS = new Set([
  ...
  'MetaLeft','MetaRight','CapsLock','Tab',
  'F1','F2','F3','F4','F5','F6','F7','F8','F9','F10','F11','F12',
  'PrintScreen','ScrollLock','Pause','Insert','Home','PageUp',
  'Delete','End','PageDown','ArrowUp','ArrowDown','ArrowLeft','ArrowRight',
  'NumLock','Escape','ContextMenu','Fn',
]);
...
function renderPrompt() {
  ...
  $('word-progress').textContent = `0 / ${promptText.trim().split(/\s+/).length} words`;
  updateProgress();
}
...
function updateStats(wpm) {
  ...
  // Accuracy & error rate
  if (totalTyped > 0) {
    const acc  = Math.max(0, (totalTyped - errors) / totalTyped * 100).toFixed(1);
    ...
    const typos = errors;
    $('accuracy-sub').innerHTML = typos === 0
...
function updateErrorLogDisplay() {
  ...
  if (!entries.length) {
    list.innerHTML = '<span class="err-badge none">No errors yet — keep going!</span>';
    return;
  }
...
function updateProgress() {
  ...
  const typedWords = promptText.slice(0, inputIndex).trim().split(/\s+/).filter(Boolean).length;
...
function finish() {
  ...
  let comment;
  if      (finalWPM >= 100) comment = '🔥 Blazing fast! You are a true keyboard warrior.';
  else if (finalWPM >=  70) comment = '⚡ Excellent speed! Most typists would envy you.';
  else if (finalWPM >=  50) comment = '✅ Solid performance! Keep practising to level up.';
  else                       comment = '🌱 Good effort! Consistency beats speed — keep at it!';
  $('result-comment').textContent = comment;
 
  // Worst keys list
  const worst = Object.entries(errorLog).sort((a, b) => b[1] - a[1]).slice(0, 6);
  const wkl   = $('worst-keys-list');
 
  if (!worst.length) {
    wkl.innerHTML = '<span class="err-badge none" style="background:rgba(74,222,128,.1);border-color:rgba(74,222,128,.2);color:var(--green)">No errors! Perfect run 🎉</span>';
  } else {
    wkl.innerHTML = worst.map(([code, count]) => {
      const label = humaniseKeyCode(code);
...
document.addEventListener('keydown', e => {
  ...
  if (e.code === 'Escape') { restart(); return; }
...
$$('.difficulty-btn').forEach(btn => {
  btn.addEventListener('click', () => {
    $$('.difficulty-btn').forEach(b => b.classList.remove('sel-easy', 'sel-medium', 'sel-hard'));
