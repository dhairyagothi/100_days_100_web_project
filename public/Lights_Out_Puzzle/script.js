...
function applyToggle(g, row, col) {
  ...
  directions.forEach(([dx, dy]) => {
    const nr = row + dx,
      nc = col + dy;
    if (nr >= 0 && nr < boardSize && nc >= 0 && nc < boardSize) g[nr][nc] = !g[nr][nc];
  });
}
