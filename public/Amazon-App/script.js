...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const searchBtns = document.querySelectorAll(".nav-search-icon");
    ...
    try {
      if (query && category && category !== "all" && category !== "null") {
        ...
        if (fsRes.ok) {
          ...
          fsFiltered = fsData.filter((p) =>
            p.title.toLowerCase().includes(query.toLowerCase()),
          );
        }
        products = [
          ...catProducts,
          ...fsFiltered.map((p) => formatFakeStoreProduct(p)),
        ];
      } else if (query) {
        ...
        let fsData = await fsRes.json();
        let fsFiltered = fsData.filter((p) =>
          p.title.toLowerCase().includes(query.toLowerCase()),
        );
        ...
        if (fsRes.ok) {
          ...
          products = [
            ...products,
            ...fsData.map((p) => formatFakeStoreProduct(p)),
          ];
        }
      } else {
...
if (window.location.pathname.endsWith("products.html")) {
  ...
  async function fetchRelatedProducts(category, currentId) {
    ...
    try {
      ...
      if (products.length > 0) {
        let html =
          '<h2>Products related to this item</h2><div class="products-grid">';
        products.forEach((p) => {
          html += `
