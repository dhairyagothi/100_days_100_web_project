document.addEventListener('DOMContentLoaded', () => {
    ...
    if (jobContainer) {
        jobContainer.addEventListener('click', (event) => {
            ...
            const heartButton = target.closest('.heart-btn');
            if (heartButton) {
                event.preventDefault();
                const icon = heartButton.querySelector('i');
                const isSaved = heartButton.classList.toggle('saved');
                icon.classList.toggle('fas', isSaved);
                icon.classList.toggle('far', !isSaved);
                heartButton.setAttribute('aria-label', isSaved ? 'Unsave job' : 'Save job');
                showToast(isSaved ? '❤️ Job saved!' : 'Job removed from saved');
                return;
            }
    ...
    jobs.forEach(job => {
        ...
        jobBox.innerHTML = `
            ...
            <div class="flex-btn">
                ...
                <button class="heart-btn" aria-label="Save job"><i class="far fa-heart"></i></button>
            </div>
        `;
