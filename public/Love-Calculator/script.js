...
function calculateLove() {
  ...
  if (!yourName || !crushName) {
    document.getElementById('result-message').textContent = 'Please enter both names!';
  ...
  if (!namePattern.test(yourName) || !namePattern.test(crushName)) {
    document.getElementById('result-message').textContent = 'Please enter valid names only!';
  ...
  setTimeout(() => {
    ...
    if (loveIndex <= 30) {
      ...
      msg = soulmateMessages[Math.floor(Math.random() * soulmateMessages.length)];
    }
