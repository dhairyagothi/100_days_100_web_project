document.addEventListener("DOMContentLoaded", () => {
  ...
  let searchHistory =
    JSON.parse(localStorage.getItem("githubAnalyzerHistory")) || [];

  const initTilt = () => {
    const elements = document.querySelectorAll(
      ".tilt-card, .search-section, .stat-box, .repo-card-mini",
    );
  ...
  const renderHistory = () => {
    ...
    if (searchHistory.length === 0) {
      historyContainer.innerHTML =
        '<span style="color: var(--text-secondary); font-size: 0.85rem;">No recent searches</span>';
      return;
    }
  ...
  const renderRepoDetails = (data) => {
    ...
    animateValue(
      document.getElementById("stat-stars"),
      0,
      data.stargazers_count,
      1500,
    );
    animateValue(
      document.getElementById("stat-forks"),
      0,
      data.forks_count,
      1500,
    );
    animateValue(
      document.getElementById("stat-issues"),
      0,
      data.open_issues_count,
      1500,
    );
    animateValue(
      document.getElementById("stat-contributors"),
      0,
      data.contributors_count,
      1500,
    );

    document.getElementById("stat-license").textContent = data.license
    ...
    document.getElementById("meta-language").textContent =
      data.language || "Multiple";
    document.getElementById("meta-size").textContent = formatBytes(
      data.size * 1024,
    );
  ...
  const renderUserRepos = (repos, username) => {
    ...
    if (repos.length === 0) {
      reposList.innerHTML =
        '<p style="grid-column: 1/-1; color: var(--text-secondary);">This user has no public repositories.</p>';
    } else {
      repos.forEach((repo) => {
