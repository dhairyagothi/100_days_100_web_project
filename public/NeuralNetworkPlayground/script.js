...
const Activations = {
  ...
  sigmoid:    { fn: x => 1/(1+Math.exp(-Math.max(-500,Math.min(500,x)))), deriv: x => { const s=1/(1+Math.exp(-Math.max(-500,Math.min(500,x)))); return s*(1-s); }},
...
function generateDataset(type, n, noise) {
  ...
  switch(type) {
    case 'circle': for(let i=0;i<n;i++){const a=Math.random()*Math.PI*2,r=Math.random();const inner=r<0.5;const rad=inner?Math.random()*0.35:0.55+Math.random()*0.4;pts.push([Math.cos(a)*rad+rand(),Math.sin(a)*rad+rand(),inner?1:0]);}break;
    case 'xor': for(let i=0;i<n;i++){const x=(Math.random()-0.5)*2,y=(Math.random()-0.5)*2;pts.push([x+rand(),y+rand(),(x*y>0)?1:0]);}break;
    case 'spiral': for(let i=0;i<n;i++){const c=i%2,t=(i/n)*3*Math.PI+c*Math.PI;const r=t/(3*Math.PI)*0.8+0.1;pts.push([r*Math.cos(t)+rand(),r*Math.sin(t)+rand(),c]);}break;
    case 'gaussian': for(let i=0;i<n;i++){const c=i%2;const cx=c?-0.4:0.4,cy=c?-0.4:0.4;pts.push([cx+(Math.random()-0.5)*0.6+rand(),cy+(Math.random()-0.5)*0.6+rand(),c]);}break;
    case 'moon': for(let i=0;i<n;i++){const c=i%2;const a=Math.random()*Math.PI;if(c===0){pts.push([Math.cos(a)+rand(),Math.sin(a)+rand()-0.2,0]);}else{pts.push([1-Math.cos(a)+rand(),1-Math.sin(a)-0.5+rand(),1]);}}break;
    default: return generateDataset('circle',n,noise);
  }
  // Normalize to [-1,1]
  ...
  pts.forEach(p=>{p[0]=(p[0]-minX)/rx*2-1;p[1]=(p[1]-minY)/ry*2-1;});
  return pts;
}
...
class NeuralNetwork {
  ...
  initWeights() {
    ...
    for (let i = 0; i < this.layers.length - 1; i++) {
      const fanIn = this.layers[i], fanOut = this.layers[i+1];
  ...
  forward(input) {
    const zs = [], as = [input.slice()];
    let a = input.slice();
    for (let l = 0; l < this.weights.length; l++) {
      const z = [], w = this.weights[l], b = this.biases[l];
  ...
  train(inputs, targets, batchSize) {
    ...
    for (let bStart = 0; bStart < indices.length; bStart += bs) {
      ...
      for (let bi = bStart; bi < bEnd; bi++) {
        ...
        const x = inputs[idx], t = targets[idx];
...
function drawBoundary() {
  const c = boundaryCanvas, ctx = bCtx;
...
function drawNetwork() {
  const c = networkCanvas, ctx = nCtx;
  const w = c.width, h = c.height;
...
function trainStep() {
  ...
  for (let s = 0; s < state.speed; s++) {
    ...
    if (state.epoch % Math.max(1, Math.floor(state.speed/5)) === 0 || state.lossHistory.length === 0) {
...
function setupEvents() {
  ...
  $('btnRegenData').addEventListener('click', () => { initData(); resetAll(); });
  ...
  // Toggles
  $('showDataToggle').addEventListener('change', e => { state.showData = e.target.checked; renderAll(); });
  $('showWeightsToggle').addEventListener('change', e => { state.showWeights = e.target.checked; renderAll(); });
