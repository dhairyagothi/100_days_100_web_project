...
function renderRowWeather(row, data) {
    ...
    cells[0].textContent = Number.isFinite(current.cloud_cover)
        ? `${Math.round(current.cloud_cover)}%`
        : '—';

    cells[1].textContent = Number.isFinite(current.apparent_temperature)
        ? `${Math.round(current.apparent_temperature)}°C`
        : '—';

    cells[2].textContent = Number.isFinite(current.relative_humidity_2m)
        ? `${Math.round(current.relative_humidity_2m)}%`
        : '—';

    cells[3].textContent = Number.isFinite(daily.temperature_2m_max?.[0])
        ? `${Math.round(daily.temperature_2m_max[0])}°C`
        : '—';

    cells[4].textContent = Number.isFinite(daily.temperature_2m_min?.[0])
    ...
    cells[7].textContent = Number.isFinite(current.temperature_2m)
        ? `${Math.round(current.temperature_2m)}°C`
        : '—';

    cells[8].textContent = Number.isFinite(current.wind_direction_10m)
        ? `${Math.round(current.wind_direction_10m)}°`
        : '—';

    cells[9].textContent = Number.isFinite(current.wind_speed_10m)
        ? `${Math.round(current.wind_speed_10m)} km/h`
        : '—';
}
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    setStatus(
        'Search for a city to load live weather data.',
        'info'
    );
