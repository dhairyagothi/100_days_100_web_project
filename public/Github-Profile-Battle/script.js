...
async function fetchUser(username) {
  try {
    ...
    if (!res.ok) {
      throw new Error(
        'Unable to fetch profile data due to a GitHub API error.'
      );
    }
...
async function startBattle() {
  ...
  try {
    ...
    const [user1, user2, repos1, repos2, events1, events2] = results.map((r) =>
    ...
    if (!user1 || !user2) {
      ...
      throw (
        failedUser ||
        new Error('Unable to fetch profile data due to a GitHub API error.')
      );
    }
