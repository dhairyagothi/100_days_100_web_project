...
const SHARE_ACTION_BUTTONS = [DOM.copyCardBtn, DOM.downloadCardBtn, DOM.shareCardBtn].filter(
  Boolean
);
...
async function fetchRepos(username) {
  ...
  while (true) {
    ...
    if (!res.ok) {
      throw new Error(`Failed to fetch repositories for ${username} (HTTP ${res.status})`);
    }
...
function renderShareCard(s1, s2, score1, score2) {
  ...
  DOM.shareResult.textContent = winner === 'Tie' ? "🤝 It's a Tie!" : `🏆 ${winner} Wins!`;
