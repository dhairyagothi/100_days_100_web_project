...
function animationStep(timestamp) {
    ...
    if (animationMode === "spring") {
        ...
        const easeFactor = animationMode === "ease" ? Math.max(0.22, 1 - currentProgress / 100) : 1;
...
function handleMilestoneSound() {
    for (let index = milestones.length - 1; index >= 0; index -= 1) {
        if (currentProgress >= milestones[index].value) {
            if (index > lastMilestoneIndex && milestones[index].value !== 0 && milestones[index].value !== 100) {
                playTick();
            }
...
function handleKeyControls(event) {
    ...
    setProgress(currentProgress + delta * direction, { keepState: true, silent: true });
}
...
function renderHistory() {
    ...
    const avg = history.reduce((total, item) => total + item.duration, 0) / history.length;
...
function drawSparkline() {
    ...
    const points = durations.map((duration, index) => {
        const x = history.length === 1 ? width / 2 : (width / (history.length - 1)) * index;
...
modeButtons.forEach((button) => {
    button.addEventListener("click", () => updateModeButtons(button.dataset.mode));
});
...
presetButtons.forEach((button) => {
    button.addEventListener("click", () => {
        ...
        setProgress(value, { keepState: true, silent: value < PROGRESS_TARGET, manual: true });
        startButton.disabled = false;
        startButton.textContent = value >= PROGRESS_TARGET ? "Restart Progress" : "Start Progress";
    });
});
...
[...modeButtons, ...presetButtons, startButton, stopButton, resetButton].forEach((button) => {
