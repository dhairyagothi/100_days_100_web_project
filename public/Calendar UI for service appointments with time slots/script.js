...
class Calendar {
    ...
    attachEventListeners() {
        ...
        this.serviceSelect.addEventListener('change', (e) => this.handleServiceSelection(e));
        this.confirmButton.addEventListener('click', () => this.handleBookingConfirmation());
        ...
        for (let day = 1; day <= totalDays; day++) {
            ...
            if (currentDate < new Date().setHours(0, 0, 0, 0)) {
                ...
                dayElement.addEventListener('mouseleave', () => {
                    if (!dayElement.classList.contains('disabled') && !dayElement.classList.contains('selected')) {
                        anime({
                            targets: dayElement,
                            scale: [1.2, 1],
                            backgroundColor: [
                ...
                dayElement.addEventListener('click', () => this.handleDateSelection(currentDate));
            }
        ...
        for (let hour = BUSINESS_HOURS.start; hour < BUSINESS_HOURS.end; hour++) {
            for (let minute = 0; minute < 60; minute += BUSINESS_HOURS.interval) {
                ...
                if (bookedAppointments.has(slotKey)) {
                    ...
                    slotElement.addEventListener('click', () => this.handleTimeSelection(timeString));
                }
    ...
    updateBookingSummary() {
        ...
        this.confirmButton.disabled = !(this.selectedService && this.selectedDate && this.selectedTime);

        anime({
            targets: [this.selectedServiceElement, this.selectedDateElement, this.selectedTimeElement],
            translateX: [-20, 0],
            opacity: [0, 1],
            delay: anime.stagger(100),
            duration: 300,
    ...
    handleBookingConfirmation() {
        ...
        const slotKey = `${this.selectedDate.toISOString().split('T')[0]}-${this.selectedTime}`;
        bookedAppointments.add(slotKey);

        this.lastConfirmed = {
            service: this.selectedService,
            serviceText: this.serviceSelect.options[this.serviceSelect.selectedIndex].text,
        ...
        alertBox.innerHTML = `
            <div class="alert-content">
                ...
                <p>Service: ${this.lastConfirmed.serviceText}</p>
                <p>Date: ${this.lastConfirmed.date.toLocaleDateString()}</p>
                <p>Time: ${this.lastConfirmed.time}</p>
    ...
    generatePDF() {
        try {
            if (!this.lastConfirmed) throw new Error('No appointment details available.');
            ...
            doc.text(`Date: ${date.toLocaleDateString()}`, 20, 105);
            ...
            doc.text('• Wear a mask and follow social distancing guidelines', 20, 165);
            
            doc.setFontSize(10);
            doc.text('This is an automated confirmation. Please keep this for your records.', 20, 250);
            
            const filename = `appointment_confirmation_${date.toISOString().split('T')[0]}.pdf`;
