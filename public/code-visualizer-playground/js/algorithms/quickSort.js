(function () {
  ...
  async function partition(array, low, high, speed, callbacks) {
    ...
    explain(callbacks, 'Choosing index ' + high + ' (value ' + pivot + ') as the pivot.');

    for (var j = low; j < high; j += 1) {
      explain(
        callbacks,
        'Comparing index ' + j + ' (value ' + array[j] + ') with pivot ' + pivot + '.'
      );
      ...
      if (array[j] <= pivot) {
        ...
        if (i !== j) {
          explain(
            callbacks,
            'Value ' + array[j] + ' is less than or equal to pivot, swapping with index ' + i + '.'
          );
    ...
    explain(callbacks, 'Placing pivot into its final sorted position at index ' + (i + 1) + '.');
    var tempB = array[i + 1];
