...
export const BADGES = [
  {
    id: "first_step",
    name: "First Step",
    desc: "Join or create your first study room",
    icon: "🎯",
    requirement: "Join a room",
  },
  {
    id: "pomodoro_complete",
    name: "Deep Focus",
    desc: "Complete your first Pomodoro focus session",
    icon: "⏱",
    requirement: "Focus session",
  },
  {
    id: "social_butterfly",
    name: "Social Scholar",
    desc: "Send 5 or more messages in room chat",
    icon: "💬",
  ...
  },
  {
    ...
    desc: "Draw something on the shared whiteboard",
    icon: "🎨",
    requirement: "Draw on whiteboard",
  },
  {
    id: "quiz_scholar",
    name: "Memory Wizard",
    desc: "Create and flip revision flashcards",
    icon: "🧙",
    requirement: "Flip flashcard",
  },
  {
    id: "task_slayer",
    name: "Goal Achiever",
    desc: "Check off a task in the session checklist",
    icon: "✓",
    requirement: "Check off task",
  },
  {
    id: "deep_focus_legend",
    name: "Focus Legend",
    desc: "Study for 60 minutes or more in total",
    icon: "🏆",
  ...
  },
  {
    id: "zen_practitioner",
    name: "Zen Master",
    desc: "Complete a guided box breathing break",
    icon: "🧘",
    requirement: "Breathing break",
  },
  {
    id: "night_owl",
    name: "Midnight Scholar",
    desc: "Study during late hours",
    icon: "🦉",
...
function initPersistedState() {
  ...
  if (savedRooms) {
    ...
    rawState.rooms = [
      {
        id: 1,
        name: "🏛 Cozy Campus Library",
        category: "General",
        description: "Quiet room for focused study. Pomodoro mode active.",
        focusTime: 25,
        creator: "Nexus",
      },
      {
        id: 2,
        name: "💻 Algorithms & Caffeine",
        category: "Computer Science",
        description: "Reviewing LeetCode patterns and system design.",
        focusTime: 45,
        creator: "Nexus",
      },
      {
        id: 3,
        name: "📐 Advanced Calculus Lab",
        category: "Mathematics",
        description: "Studying Fourier transforms. Flashcards enabled.",
        focusTime: 50,
        creator: "Nexus",
      },
    ];
      ...
      callback: async (tokenResponse) => {
        if (tokenResponse && tokenResponse.access_token) {
          ...
          if (res.ok) {
            ...
            state.user = {
              username:
                googleUser.name || googleUser.given_name || "Google Scholar",
            ...
            if (state.backendMode) {
              try {
                await executeAuthSignup(
                  state.user.username,
                  state.user.email,
                  "google-oauth-password-bypass",
                  state.user.avatar,
                );
              } catch (e) {
...
function emulateGoogleLogin() {
  ...
  if (state.backendMode) {
    executeAuthSignup(
      ...
      .catch(() => {
        executeAuthLogin(randomEmail, "google-oauth-password-bypass").catch(
          console.error,
        );
      });
  }
  ...
  alert(
    "🔗 Emulated Google Authentication Success!\nLogged in as: " +
      randomName +
      " (" +
      randomEmail +
      ")",
...
export async function executeAuthSignup(username, email, password, avatar) {
  if (state.backendMode) {
    ...
    const users = JSON.parse(
      localStorage.getItem("nexus_registered_users") || "[]",
    );
...
export async function executeJoinRoomByCode(code) {
  if (state.backendMode) {
    try {
      const res = await fetch(`${API_BASE_URL}/rooms/code/${code}`, {
        headers: getHeaders(),
      });
      if (res.ok) {
...
export async function fetchRoomNotepad(roomId) {
  if (state.backendMode) {
    try {
      const res = await fetch(`${API_BASE_URL}/notes/${roomId}`, {
        headers: getHeaders(),
      });
      if (res.ok) {
    ...
    if (!state.notepad[roomId]) {
      state.notepad[roomId] =
        "Welcome to your synced study notepad! Write group summaries or draft code together here...";
    }
  }
}
    ...
    setTimeout(() => {
      ...
      if (query.contains("flashcard") || query.contains("quiz")) {
        if (
          ...
          query.contains("anatomy")
        ) {
          replyText =
            "🩺 **Jaypee Study AI**: I have generated 3 cardiac anatomy flashcards for your revision deck!";
          flashcardsList.push({
          ...
          replyText =
            "🩺 **Jaypee Study AI**: Here are 3 general medical physiology flashcards for your deck!";
          flashcardsList.push({
        ...
        replyText =
          "❤️ **Cardiovascular System Summary:**\n" +
        ...
        replyText =
          "👋 Hello! I am the **Jaypee Medical Study AI** assistant.\n" +
...
export function addXP(amount) {
  ...
  if (newLevel > oldLevel) {
    ...
    if (state.activeRoom) {
      sendChatMessage(
        `✨ leveled up to Level ${newLevel}! Keep pushing!`,
        "system",
      );
    }
  }
...
export function unlockBadge(badgeId) {
  ...
  if (!state.user.badgesUnlocked.includes(badgeId)) {
    ...
    if (state.activeRoom) {
      ...
      sendChatMessage(
        `🏆 unlocked the achievement badge: "${badge.name}" ${badge.icon}!`,
        "system",
      );
    }
    return true;
    ...
    switch (type) {
      ...
      case "WHITEBOARD_DRAW":
        window.dispatchEvent(
          new CustomEvent("whiteboard-draw-stroke", { detail: data }),
        );
        break;
...
export function sendChatMessage(text, type = "text") {
  ...
  if (type === "text") {
    const userMsgCount = state.chat[roomId].filter(
      (m) => m.sender === state.user.username && m.type === "text",
    ).length;
    if (userMsgCount >= 5) {
...
export function resetTimer() {
  ...
  const duration =
    state.timer.mode === "focus" ? state.activeRoom.focusTime * 60 : 300;
  state.timer = {
    mode: state.timer.mode,
    duration,
...
function handleIncomingNotepadUpdate(roomId, text) {
  ...
  window.dispatchEvent(
    new CustomEvent("notepad-sync-update", { detail: text }),
  );
}
