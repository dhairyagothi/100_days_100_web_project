...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  document
    .getElementById("auth-back-to-home")
  ...
  if (contactForm) {
    contactForm.addEventListener("submit", (e) => {
      ...
      alert(
        `Thank you, ${name}! Your message has been received. Our team will reach out to you shortly.`,
      );
      contactForm.reset();
    });
  }
...
function renderHeaderUserPanel() {
  if (state.user) {
    ...
    document.getElementById("dropdown-email").textContent =
      state.user.email || "offline@nexus.com";
    document.getElementById("dropdown-user-avatar").textContent =
      state.user.avatar;
    ...
    document.getElementById("dropdown-xp-progress-text").textContent =
      `${xpProgress} / 100 XP`;
    const progressFill = document.getElementById(
      "dropdown-level-progress-fill",
    );
    if (progressFill) {
    ...
    if (unlockedList.length === 0) {
      ...
      badgesListContainer.innerHTML = unlockedList
        .map((badgeId) => {
...
async function initialPageBoot() {
  try {
    ...
    bootLoader.querySelector("p").textContent =
...
function startSoundscapeVisualizer() {
  ...
  function draw() {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      const grad = visualizerCtx.createLinearGradient(
        0,
        height,
        0,
        height - barHeight,
