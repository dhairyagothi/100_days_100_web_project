...
export function initRoomComponent() {
  ...
  const btnDistractionFree = document.getElementById(
    "btn-toggle-distraction-free",
  );
  ...
  btnInvite.addEventListener("click", async () => {
    ...
    const inviteCode =
      state.activeRoom.roomCode || `NEXUS-ROOM-${state.activeRoom.id}`;
...
export function renderRoomView() {
  ...
  if (state.activeRoom) {
    ...
    // Update labels
    document.getElementById("room-display-name").textContent =
      state.activeRoom.name;
    document.getElementById("room-display-category").textContent =
      state.activeRoom.category;
    document.getElementById("room-display-code").textContent =
      `Code: ${state.activeRoom.roomCode || "N/A"}`;
...
export function renderMembersList() {
  ...
  listContainer.innerHTML = state.activeMembers
    .map((member) => {
