...
export function initChatComponent() {
  ...
  formMedicalAi.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      removeMedicalAiTypingIndicator(typingId);
      appendMedicalAiMessage(
        "⚠️ Failed to reach medical AI service. Connecting to offline index.",
        "system",
      );
      console.error(err);
    }
  ...
  container.innerHTML = messages
    .map((msg) => {
      ...
      <div class="${bubbleClass}">
        <div class="chat-meta">
          <span class="chat-sender">${escapeHTML(msg.avatar)} ${escapeHTML(msg.sender)}</span>
