...
const JAYPEE_BOOKS = {
  Sciences: [
    {
      title: "Basics of Medical Physiology",
      author: "D. Venkatesh, H.H. Sudhakar",
      edition: "3rd Edition",
      desc: "A highly acclaimed physiology manual mapped to clinical competencies for medical undergraduates.",
    },
    {
      title: "Review of Pharmacology",
      author: "Gobind Rai Garg, Sparsh Gupta",
      edition: "15th Edition",
      desc: "The ultimate pharmacology textbook guide for PG entrance exams and clinical references.",
    },
    {
      title: "Ghai Essential Pediatrics",
      author: "Vinod K. Paul, Arvind Bagga",
      edition: "10th Edition",
      desc: "The definitive reference textbook for pediatric care, clinical practice, and diagnostics.",
    },
  ],
  "Computer Science": [
    {
      title: "Object-Oriented Programming with Java",
      author: "Jaypee Technical Board",
      edition: "2nd Edition",
      desc: "Core OOP constructs, threads, exception handling, and GUI development principles.",
    },
    {
      title: "Database Systems & Engineering",
      author: "A. K. Sharma",
      edition: "1st Edition",
      desc: "Design schemas, normalizations, relational calculus, and query optimization methods.",
    },
  ],
  Mathematics: [
    {
      title: "Textbook of Engineering Mathematics",
      author: "N. P. Bali, Manish Goyal",
      edition: "9th Edition",
      desc: "Detailed math guides for vector calculus, complex variables, and differential equations.",
    },
  ],
  General: [
    {
      title: "Basics of Human Anatomy",
      author: "Jaypee Brothers Editorial Team",
      edition: "5th Edition",
      desc: "Comprehensive anatomical review textbook featuring detailed plates and dissection guides.",
    },
  ],
};
...
export function handleChatCommand(messageText) {
  ...
  if (clean === "/quote") {
    const randomIdx = Math.floor(getSecureRandom() * MOTIVATIONAL_QUOTES.length);
    sendChatMessage(
      `💡 **Motivational Quote**: "${MOTIVATIONAL_QUOTES[randomIdx]}"`,
      "bot-local",
    );
    return true;
  }
  ...
  if (clean === "/stretch") {
    const randomIdx = Math.floor(getSecureRandom() * STRETCHES.length);
    sendChatMessage(STRETCHES[randomIdx], "bot-local");
    return true;
  }

  if (clean === "/breathe") {
    const randomIdx = Math.floor(getSecureRandom() * BREATHING_TIPS.length);
    sendChatMessage(
      BREATHING_TIPS[randomIdx] || BREATHING_TIPS[0],
      "bot-local",
    );
    return true;
  }
...
export function startCompanionSimulator() {
  ...
  companionInterval = setInterval(() => {
    ...
    if (getSecureRandom() > 0.25) return;
    ...
    if (currentSubject && currentSubject !== "General") {
      pool = COMPANIONS.filter(
        (c) => c.subject === currentSubject || c.subject === "General",
      );
    }
    const companion = pool[Math.floor(getSecureRandom() * pool.length)];
    ...
    if (!state.activeMembers.some((m) => m.username === companion.username)) {
      ...
      const enterMsg =
        enterMessages[Math.floor(getSecureRandom() * enterMessages.length)];
    ...
    if (currentSubject === "Sciences" || currentSubject === "General") {
      messages.push(
        "Reading through the respiratory physiology slides. Fascinating stuff!",
      );
      messages.push(
        "Currently studying the anatomical structures of the cardiac valves. So much to memorize!",
      );
    }

    const randomMsg = messages[Math.floor(getSecureRandom() * messages.length)];
