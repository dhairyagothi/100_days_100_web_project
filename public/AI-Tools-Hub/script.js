...
const toolsData = [
  {
    ...
    description:
      'Your AI pair programmer. Get code suggestions in real-time as you write.',
  ...
  },
  {
    ...
    category: 'Design & Images',
    description:
      "OpenAI's latest image generation model with stunning quality.",
    url: 'https://openai.com/dall-e-3',
    pricing: 'Pay-as-you-go',
    api: 'Yes',
    features:
      'Image generation, Text-to-image, High resolution, Quality options',
  ...
  },
  {
    ...
    category: 'Writing & Content',
    description:
      "Anthropic's AI assistant for thoughtful, harmless, and helpful interactions.",
...
function renderRecent() {
  ...
  recentGrid.innerHTML = recentTools
    .map(
      (tool) => `
    <div class="tool-card glass-card" data-tool-id="${tool.id}">
...
function renderComparison() {
  ...
  if (!tool1Id || !tool2Id) {
    comparisonTable.innerHTML =
      '<p class="comparison-empty">Please select both tools to compare.</p>';
...
function initAnimations() {
  const cardsToAnimate = document.querySelectorAll(
    '.tool-card, .category-card, .stat-card, .why-card, .trend-card, .collection-card'
  );
...
document.addEventListener('click', (e) => {
  ...
  if (favoriteBtn) {
    ...
    const toolCard = favoriteBtn.closest('.tool-card');
  ...
  if (toolCard && !e.target.closest('a') && !e.target.closest('button')) {
    addToRecent(toolCard.dataset.toolId);
  }
});
