...
const clearHighlight = () => {
  document.querySelectorAll('.morse-token').forEach((s) => s.classList.remove('active-token'));
};
...
const togglePlay = async () => {
  ...
  const morseText = mode === 'encode' ? el('output').textContent : el('input').value;
  ...
  for (const char in MORSE_MAP) {
    ...
    item.addEventListener('click', () => {
      ...
      if (mode === 'encode') {
        ...
        inputEl.value = current ? current.trimEnd() + ' ' + MORSE_MAP[char] : MORSE_MAP[char];
      }
