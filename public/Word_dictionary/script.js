...
function renderMeaning(idx) {
  ...
  const def = m.definitions[0]?.definition || 'No explicit content definitions available.';
...
function renderChips(container, words, cls) {
  ...
  if (!words.length) {
    ...
    span.textContent = cls === 'chip-syn' ? 'No synonyms categorized' : 'None available';
...
if (SpeechRecognition) {
  ...
  recognition.onend   = () => { if (isListening) toggleListening(true); };
} else {
