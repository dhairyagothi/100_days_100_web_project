...
function bindEvents() {
  ...
  // Form meta inputs (with settings panel sync)
  $("formTitle").addEventListener("input", (e) => {
    const val = e.target.value;
    formSettings.title = val || "Untitled Form";
    $("settingsFormTitle").value = val;
  ...
  $("formDesc").addEventListener("input", (e) => {
    const val = e.target.value;
    formSettings.description = val;
    $("settingsFormDesc").value = val;
    pvDesc.textContent = val;
    scheduleAutoSave();
  });

  $("settingsFormTitle").addEventListener("input", (e) => {
    const val = e.target.value;
    formSettings.title = val || "Untitled Form";
    $("formTitle").value = val;
    pvTitle.textContent = formSettings.title;
  ...
  $("settingsFormDesc").addEventListener("input", (e) => {
    const val = e.target.value;
    formSettings.description = val;
    $("formDesc").value = val;
    pvDesc.textContent = val;
  ...
  $("savedFormsBtn").addEventListener("click", openSavedFormsModal);

  $("closeSavedForms").addEventListener("click", closeSavedFormsModal);

  $("savedFormsSearch").addEventListener("input", renderSavedForms);

  // Mobile drawer controls
  $("openBuilderBtn").addEventListener("click", openBuilderDrawer);
  $("openFieldsBtn").addEventListener("click", openFieldsDrawer);
  $("closeBuilderDrawer").addEventListener("click", closeDrawers);
  $("closeFieldsDrawer").addEventListener("click", closeDrawers);
...
function renameSavedForm(id) {
  ...
  const newName = prompt("Enter new form name:", form.title);
...
function addField() {
  ...
  // Flash success & auto-close drawer on mobile
...
function renderAll() {
  ...
  const mobCountEl = $("mobileFieldCount");
...
// ─── Theme & Color ────────────────────────────────────────────────────
function applyTheme() {
  previewFrame.setAttribute("data-theme", formSettings.theme);
}
...
function saveEdit() {
  ...
  field.validation = {
    minLen:
      $("editMinLen").value !== "" ? parseInt($("editMinLen").value) : null,
    maxLen:
      $("editMaxLen").value !== "" ? parseInt($("editMaxLen").value) : null,
    minNum:
      $("editMinNum").value !== "" ? parseFloat($("editMinNum").value) : null,
    maxNum:
      $("editMaxNum").value !== "" ? parseFloat($("editMaxNum").value) : null,
...
function generateHTML() {
  ...
  fields.forEach((field) => {
    ...
    if (v.regex)
      validAttrs += ` pattern="${escapeAttr(v.regex)}" title="${escapeAttr(v.regexMsg)}"`;

    if (field.type === "select") {
      html += `    <select${req}>\n`;
      if (field.placeholder)
        html += `      <option value="" disabled selected>${escapeHtml(field.placeholder)}</option>\n`;
...
function applyPayload(payload) {
  ...
  $("settingsFormTitle").value = formSettings.title;
  $("settingsFormDesc").value = formSettings.description;
  ...
  previewInner.style.setProperty(
    "--preview-radius",
...
function openSavedFormsModal() {
  $("savedFormsModal").classList.remove("hidden");
  renderSavedForms();
}

function closeSavedFormsModal() {
  $("savedFormsModal").classList.add("hidden");
}

function renderSavedForms() {
  const container = $("savedFormsList");

  const search = $("savedFormsSearch").value.toLowerCase();
  ...
  container.innerHTML = filtered
    .map(
      (form) => `
    <div class="saved-form-card">

      <div class="saved-form-title">
        ${form.title}
      </div>

      <div class="saved-form-time">
        ${form.timestamp}
      </div>

      <div class="saved-form-actions">

        <button onclick="loadSavedForm(${form.id})">
          Open
        </button>

        <button onclick="renameSavedForm(${form.id})">
          Rename
        </button>

        <button onclick="deleteSavedForm(${form.id})">
          Delete
        </button>
...
function openBuilderDrawer() {
  ...
  $("openBuilderBtn").classList.add("active");
  $("openFieldsBtn").classList.remove("active");
}
...
function openFieldsDrawer() {
  ...
  $("drawerBackdrop").classList.add("active");
  $("openFieldsBtn").classList.add("active");
  $("openBuilderBtn").classList.remove("active");
}
