...
function renderTags() {
  const tags = [
    'all',
    ...new Set(state.notes.filter((note) => !note.trashed).map((note) => note.tag)),
  ];
...
function handleCardAction(event) {
  ...
  if (action === 'favorite')
    updateNote(id, (item) => ({ ...item, favorite: !item.favorite }), 'Favorite updated.');
  if (action === 'archive')
    updateNote(id, (item) => ({ ...item, archived: !item.archived }), 'Archive updated.');
