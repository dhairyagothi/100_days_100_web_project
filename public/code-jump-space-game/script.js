...
function update() {
  ...
  while (platformArray.length > 0 && platformArray[0].y - cameraY > boardHeight + 100) {
    platformArray.shift();
    newPlatform();
  }
...
function handleClick(e) {
  ...
  // START
  if (!gameStarted) {
    if (x > bx && x < bx + 120 && y > boardHeight / 2 && y < boardHeight / 2 + 50) {
      startGame();
    }
    return;
  }
  ...
  if (gameOver) {
    if (x > bx && x < bx + 120 && y > boardHeight / 2 + 20 && y < boardHeight / 2 + 70) {
      resetGame();
    }
    return;
  }
