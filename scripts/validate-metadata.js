...
function addFieldError(errors, index, project, fieldName, message) {
  errors.push(
    `${formatProjectLabel(index, project)}: "${fieldName}" ${message}`,
  );
}
...
function validateExternalUrl(projectPath, index, project, errors) {
  ...
  if (
    isGitHubUrl(parsedUrl) &&
    getGitHubRepository(parsedUrl.pathname) !== EXPECTED_GITHUB_REPOSITORY
  ) {
    addFieldError(
      errors,
      index,
      project,
      "projectPath",
      "GitHub URLs must point to this repository",
    );
  }
}
...
function validateLocalPath(projectPath, index, project, errors) {
  ...
  try {
    ...
    addFieldError(
      ...
      project,
      "projectPath",
      `failed to decode URI component for "${relPath}"`,
    );
    return;
  }
  ...
  if (!fs.existsSync(resolvedPath)) {
    addFieldError(
      ...
      project,
      "projectPath",
      `local path "${relPath}" does not exist in the repository`,
    );
  }
}
...
function validateProjectPath(projectPath, index, project, errors) {
  ...
  if (URL_SCHEME_RE.test(rawPath)) {
    addFieldError(
      ...
      project,
      "projectPath",
      "must use http(s) or a local relative path",
    );
    return;
  }
...
try {
    ...
    if (project.projectNo !== undefined && project.projectNo !== null) {
      if (typeof project.projectNo !== "number") {
        addFieldError(
          ...
          project,
          "projectNo",
          `must be a number, got "${typeof project.projectNo}"`,
        );
      } else if (
        !Number.isInteger(project.projectNo) ||
        project.projectNo <= 0
      ) {
        addFieldError(
          errors,
        ...
        addFieldError(
          ...
          project,
          "projectNo",
          `duplicates day number "${project.projectNo}"`,
        );
      } else {
    ...
    if (
      ...
      if (typeof project.projectName !== "string") {
        addFieldError(
          ...
          project,
          "projectName",
          `must be a string, got "${typeof project.projectName}"`,
        );
    ...
    if (project.techStack !== undefined && project.techStack !== null) {
      if (!Array.isArray(project.techStack)) {
        addFieldError(
          ...
          project,
          "techStack",
          `must be an array, got "${typeof project.techStack}"`,
        );
    ...
    if (
      ...
      if (typeof project.projectType !== "string") {
        addFieldError(
          ...
          project,
          "projectType",
          `must be a string, got "${typeof project.projectType}"`,
        );
        ...
        addFieldError(
          ...
          project,
          "projectType",
          `must be one of: ${Array.from(VALID_PROJECT_TYPES).join(", ")}`,
        );
      }
    }
    ...
    if (
      ...
      if (typeof project.difficulty !== "string") {
        addFieldError(
          ...
          project,
          "difficulty",
          `must be a string, got "${typeof project.difficulty}"`,
        );
    ...
    if (
      ...
      if (typeof project.projectPath !== "string") {
        addFieldError(
          ...
          project,
          "projectPath",
          `must be a string, got "${typeof project.projectPath}"`,
        );
      } else {
        if (seenProjectPaths.has(project.projectPath)) {
          addFieldError(
            ...
            project,
            "projectPath",
            `duplicates path "${project.projectPath}"`,
          );
        } else {
  ...
  // Check if existing project contributors have been modified compared to the baseline from Main branch.
  const isBypass = process.env.BYPASS_CONTRIBUTOR_CHECK === "true";
  if (!isBypass) {
    let baselineProjects = null;
    try {
      const { execSync } = require("child_process");
      const branchesToTry = ["origin/Main", "Main", "origin/master", "master"];
      let baselineData = null;
      for (const branch of branchesToTry) {
        try {
          baselineData = execSync(`git show ${branch}:projects.json`, {
            stdio: ["pipe", "pipe", "ignore"],
            encoding: "utf8",
          });
          if (baselineData) {
            console.log(
              `Comparing against baseline projects from branch: ${branch}`,
            );
            break;
          }
        } catch (_) {}
      ...
      if (baselineData) {
        baselineProjects = JSON.parse(baselineData);
      }
    } catch (err) {
      console.warn(
        "Warning: Could not fetch baseline projects.json from git. Skipping change detection for existing project contributors.",
      );
    }

    if (baselineProjects && Array.isArray(baselineProjects)) {
      const baselineByNo = new Map();
      const baselineByPath = new Map();
      baselineProjects.forEach((p) => {
        if (p.projectNo !== undefined) {
          baselineByNo.set(p.projectNo, p);
        }
        if (p.projectPath) {
          baselineByPath.set(p.projectPath, p);
        }
      });
      ...
      projects.forEach((project, index) => {
        if (project) {
          ...
          if (
            ...
            baselineByNo.has(project.projectNo)
          ) {
            baseProject = baselineByNo.get(project.projectNo);
          } else if (
            project.projectPath &&
            baselineByPath.has(project.projectPath)
          ) {
            baseProject = baselineByPath.get(project.projectPath);
          }

          if (
            baseProject &&
            baseProject.contributor &&
            project.contributor !== baseProject.contributor
          ) {
            errors.push(
              `${formatProjectLabel(index, project)}: "contributor" field was modified from "${baseProject.contributor}" to "${project.contributor}". Modifying existing project contributors is not allowed.`,
            );
          }
        }
      });
    ...
    console.log("Bypassing existing project contributor modification check.");
  }
  ...
  console.log(
    "Success: projects.json registry is valid and verified perfectly!",
  );
  process.exit(0);
} catch (error) {
